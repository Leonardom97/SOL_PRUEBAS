...
try {
    ...
    // prefer common names
    $candidates = ['nombre','name','titulo','descripcion','tema','label'];
    $nameCol = null;
    ...
    // ensure estado column exists (we set it to 0)
    ...
    if (!preg_match('/^[a-zA-Z0-9_]+$/',$nameCol)) {
        api_log("unsafe column name: $nameCol");
    ...
    api_log("agregar_cap_tema error: ".$e->getMessage());
