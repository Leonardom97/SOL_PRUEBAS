...
if(!$inc||!isset($pg)||!($pg instanceof PDO)){ api_log("update cap_proceso DB missing"); respond(['success'=>false,'error'=>'DB error']); }
...
try {
    $table='cap_proceso';
    $prefs=['nombre','name','descripcion','titulo','proceso'];
    $stmt=$pg->prepare("SELECT column_name,data_type FROM information_schema.columns WHERE table_name=:t"); $stmt->execute([':t'=>$table]);
    ...
    if(!$col){ api_log("update_cap_proceso: no text col"); respond(['success'=>false,'error'=>'Tabla no preparada']); }
    if(!preg_match('/^[a-zA-Z0-9_]+$/',$col)){ api_log("update_cap_proceso: columna insegura {$col}"); respond(['success'=>false,'error'=>'Configuración inválida']); }
...
} catch(Exception $e){ api_log("update_cap_proceso error: ".$e->getMessage()); respond(['success'=>false,'error'=>'Error al actualizar']); }