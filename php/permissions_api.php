...
// Modo debug - establecer en false en producción
...
function extractDataRolesFromFile($file_path, $base_path)
{
    ...
    // Comprobar tamaño del archivo (máx 5MB)
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['update_catalog'])) {
        ...
        try {
            ...
            $logger->log(
                ...
                'Actualización de catálogo de recursos',
                ['updated_count' => $count]
            );
    ...
    if (isset($_GET['my_permissions'])) {
        ...
        $tipo_usuario = $_SESSION['tipo_usuario'] ?? 'admin'; // Default fallback

        try {
            $user_roles = [];

            // CASO ESPECIAL: Colaboradores
            if ($tipo_usuario === 'colaborador') {
                // Los colaboradores siempre tienen rol 'usuario' (ID 2)
                $user_roles = [
                    ['id' => 2, 'nombre' => 'usuario']
                ];
                // Si tenemos roles en sesión, usarlos preferiblemente
                if (isset($_SESSION['roles']) && !empty($_SESSION['roles'])) {
                    $user_roles = $_SESSION['roles'];
                }
            }
            // CASO NORMAL: Usuarios Admin/Sistema
            else {
    ...
    if (isset($_GET['check_access']) && isset($_GET['page'])) {
        ...
        try {
            ...
            // Buscar coincidencia exacta o coincidencia sin barra inicial
            // Esto maneja casos donde la BD tiene 'm_capacitaciones/...' y la entrada es '/m_capacitaciones/...'
            ...
            // Si no hay coincidencia exacta, intentar coincidencia insensible a mayúsculas
            if (!$has_permission) {
            ...
            // Si aún no hay coincidencia, intentar coincidencia por ruta de módulo (si la página está en un directorio de módulo)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $logger->log(
            ...
            'Actualización de permisos de rol',
            ['role_id' => $rol_id, 'permission_count' => $inserted_count]
        );
