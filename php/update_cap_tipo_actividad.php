...
if(!$inc||!isset($pg)||!($pg instanceof PDO)){ api_log("update cap_tipo_actividad DB missing"); respond(['success'=>false,'error'=>'DB error']); }
...
try{
    ...
    $prefs=['nombre','name','descripcion','titulo','tipo','actividad'];
    ...
    if(!$col){ api_log("update_cap_tipo_actividad: no text col"); respond(['success'=>false,'error'=>'Tabla no preparada']); }
    if(!preg_match('/^[a-zA-Z0-9_]+$/',$col)){ api_log("update_cap_tipo_actividad: columna insegura {$col}"); respond(['success'=>false,'error'=>'Configuración inválida']); }
...
} catch(Exception $e){ api_log("update_cap_tipo_actividad error: ".$e->getMessage()); respond(['success'=>false,'error'=>'Error al actualizar']); }