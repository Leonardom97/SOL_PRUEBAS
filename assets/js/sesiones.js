...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // NEW: Actions refresh
    const rab2 = document.getElementById('refreshActionsBtn');
    if (rab2) rab2.addEventListener('click', loadActionHistory);
  ...
  // --- NEW: Load Action History ---
  async function loadActionHistory() {
    try {
      const tbody = document.getElementById('actionHistoryTable');
      if (tbody) tbody.innerHTML = '<tr><td colspan="6" class="text-center"><div class="spinner-border" role="status"><span class="visually-hidden">Cargando...</span></div></td></tr>';

      const res = await fetch('php/session_management_api.php?action=get_logs', { cache: 'no-store' });
      const data = await res.json();

      if (!data.success || !Array.isArray(data.logs) || data.logs.length === 0) {
        if (tbody) tbody.innerHTML = '<tr><td colspan="6" class="text-center">No hay acciones registradas</td></tr>';
        return;
      }

      const isAdminView = data.is_admin_view;

      if (!tbody) return;

      tbody.innerHTML = data.logs.map(log => {
        const userDisplay = log.user_name ? `${escapeHtml(log.user_name)} (${escapeHtml(log.user_identifier)})` : 'Sistema/Desconocido';

        // Format Details: Check if it's JSON
        let detailsHtml = '-';
        if (log.details) {
          try {
            // Simple check if it looks like JSON or is a string
            const d = typeof log.details === 'string' ? JSON.parse(log.details) : log.details;
            detailsHtml = `<small>${escapeHtml(JSON.stringify(d)).substring(0, 50)}${JSON.stringify(d).length > 50 ? '...' : ''}</small>`;
          } catch (e) {
        ...
        return `
            <tr>
                <td>${userDisplay}</td>
                <td><span class="badge bg-secondary">${escapeHtml(log.action_type)}</span></td>
      ...
      const tbody = document.getElementById('actionHistoryTable');
      if (tbody) tbody.innerHTML =
        '<tr><td colspan="6" class="text-center text-danger">Error al cargar historial de acciones</td></tr>';
    }
  }
