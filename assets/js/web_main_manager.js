...
(function () {
    ...
    /**
     * Update user name in navbar from session storage
     */
    function updateNavbarUser() {
        try {
            const userData = JSON.parse(sessionStorage.getItem('userData'));
            const userEl = document.getElementById('usuarioNombre');
            if (userEl && userData && userData.nombre) {
                userEl.textContent = userData.nombre;
            }
        } catch (e) {
            console.error('Error updating navbar user:', e);
        }
    }
    ...
    async function handleFormSubmit(e) {
        ...
        try {
            // Determine method based on ID (0 or empty = POST/Create, otherwise PUT/Update)
