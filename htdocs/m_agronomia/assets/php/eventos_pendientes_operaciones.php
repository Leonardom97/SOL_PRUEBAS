<?php
// SSE en tiempo real (filtrable por ?entidad=...)
...
function contar(PDO $pg, ?string $entidad): array {
  ...
  foreach ($cand as $t) {
    ...
    if (isset($map['verificacion'])) {
      $where = "LOWER(COALESCE(\"{$map['verificacion']}\", 'pendiente')) = 'pendiente'";
...
$inicio = time(); $timeout = 300; $intervalo = 5;
while (!connection_aborted()) {
  if (time() - $inicio > $timeout) { sse_send('end',['reason'=>'timeout']); break; }
  try {
    $estado = contar($pg, $ent ?: null); $t = (int)$estado['total'];
    if ($t !== $ultimo) { $ultimo = $t; sse_send('update',$estado); }
    else { sse_send('ping',['ts'=>time()]); }
  } catch (Throwable $e) {
