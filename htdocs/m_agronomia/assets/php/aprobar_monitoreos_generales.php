...
try{
  ...
  $pk=get_pk_column($pg_temp,$schema,$table) ?: 'monitoreos_generales_id';
  ...
  $cols=array_keys($row);
  $set=[]; $valsUpd=[];
  foreach($cols as $c){ if($c===$pk) continue; $set[] = qident($c)." = ?"; $valsUpd[]=$row[$c]; }
  $valsUpd[]=$row[$pk];
  $u=$pg_main->prepare("UPDATE public.monitoreos_generales SET ".implode(', ',$set)." WHERE ".qident($pk)." = ?");
  $u->execute($valsUpd);
  if($u->rowCount()===0){
    $colsQ=array_map('qident',$cols); $ph=array_fill(0,count($cols),'?'); $valsIns=array_map(fn($c)=>$row[$c],$cols);
    $pg_main->prepare("INSERT INTO public.monitoreos_generales (".implode(', ',$colsQ).") VALUES (".implode(', ',$ph).")")->execute($valsIns);
  }
  $pg_main->commit();
