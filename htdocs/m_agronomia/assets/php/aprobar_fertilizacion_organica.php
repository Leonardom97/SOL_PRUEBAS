...
try{
  ...
  $input=read_input();
  $id=trim((string)($input['fertilizacion_organica_id'] ?? $input['id'] ?? ''));
  if($id==='') err('fertilizacion_organica_id es requerido');

  $meta=$pg_temp->query("SELECT table_schema, table_name FROM information_schema.tables WHERE table_schema NOT IN ('pg_catalog','information_schema') AND lower(table_name) IN ('fertilizacion_organica','fertilizacion_organica_temp') ORDER BY CASE WHEN lower(table_name) LIKE '%_temp' THEN 1 ELSE 0 END LIMIT 1")->fetch(PDO::FETCH_ASSOC);
  if(!$meta) err('No existe tabla temporal de fertilizacion_organica');
  ...
  $pk=get_pk_column($pg_temp,$schema,$table) ?: 'fertilizacion_organica_id';
  ...
  $valsUpd[]=$row[$pk];
  $u=$pg_main->prepare("UPDATE public.fertilizacion_organica SET ".implode(', ',$set)." WHERE ".qident($pk)." = ?");
  $u->execute($valsUpd);
  if($u->rowCount()===0){
    $colsQ=array_map('qident',$cols); $ph=array_fill(0,count($cols),'?'); $valsIns=array_map(fn($c)=>$row[$c],$cols);
    $pg_main->prepare("INSERT INTO public.fertilizacion_organica (".implode(', ',$colsQ).") VALUES (".implode(', ',$ph).")")->execute($valsIns);
  }
  $pg_main->commit();
