(function () {
  const columnas = [
    ...
    'tipo_labor', 'contratista', 'codigo', 'colaborador', 'personas', 'hora_entrada', 'hora_salida',
    'linea_entrada', 'linea_salida', 'cantidad', 'unidad', 'maquina', 'tractorista', 'nuevo_operario', 'Verificacion'
  ];
  ...
  window.filtrarDatosTabla3 = () => allData;
  window.allTabla3Data = allData;
  ...
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    const endpoint = `assets/php/conexion_oficios_varios_palma.php?${params}`;
    const resp = await fetch(endpoint);
    if (!resp.ok) throw new Error('Error al cargar oficios_varios_palma');
  ...
  function renderTabla(datos) {
    ...
    const tbody = document.getElementById('tbody-oficios-varios-palma');
    if (!tbody) return;

    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td><input type="checkbox" class="fila-tabla3-chk fila-oficios-chk md-checkbox" data-row="${idx}"></td>`;
      ...
      if (fechaCorte && fechaFila) {
        if (fechaFila < fechaCorte) {
          ...
          btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.oficios_varios_palma_id}"><i class="fa fa-pen"></i></button>`;
        }
      } else {
        btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.oficios_varios_palma_id}"><i class="fa fa-pen"></i></button>`;
      ...
      const acciones = `
        ${btnEditar}
        <button class="md-btn md-btn-icon btn-visualizar" title="Visualizar" data-id="${row.oficios_varios_palma_id}"><i class="fa fa-eye"></i></button>
        ${btnEstado}
      `;
    ...
    tbody.querySelectorAll('.btn-editar').forEach(btn => {
      btn.onclick = function () {
        ...
        const registro = allData.find(row => row.oficios_varios_palma_id == id);
    ...
    if (esAdmin || esAuxAgronomico) {
      tbody.querySelectorAll('.btn-aprobar').forEach(btn => {
        btn.onclick = async function () {
          const id = this.getAttribute("data-id");
          const registro = allData.find(r => r.oficios_varios_palma_id == id);
          if (!registro) return;
          const payload = { ...registro, Verificacion: 'aprobado' };
          try {
            const resp = await fetch('assets/php/actualizar_oficios_varios_palma.php', {
              method: 'POST',
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas.map(col => {
      const readonly = (col === 'oficios_varios_palma_id' || col === 'codigo' || soloLectura) ? "readonly" : "";
      return `
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const select = document.getElementById('limitSelect3');
    if (select) {
    ...
    const filtrosInputs = document.querySelectorAll('#tabla-asistencias thead input[data-col]');
    ...
    const btnClear = document.getElementById('clearFiltersBtn3');
    if (btnClear) {
    ...
    if (selectAll) {
      selectAll.addEventListener('change', function() {
        document.querySelectorAll('.fila-tabla3-chk').forEach(chk => chk.checked = this.checked);
      });
    }

    document.querySelectorAll('#tabla-asistencias thead .icon-sort').forEach(flecha => {
      flecha.addEventListener('click', function () {
        ...
        document.querySelectorAll('#tabla-asistencias thead .icon-sort').forEach(f=>f.classList.remove('active'));
