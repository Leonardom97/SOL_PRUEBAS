// --- DEFINICIÓN DE TABS ---
const TABS = [
  { key: "capacitaciones", label: "Recoleccion Fruta", icon: "chalkboard-teacher" },
  { key: "reuniones", label: "Mantenimientos", icon: "users" },
  { key: "asistencias", label: "Oficios Varios Palmas", icon: "clipboard-check" },
  { key: "inventarios", label: "Fertilizacion Organica", icon: "warehouse" },
  { key: "monitoreos-generales", label: "Monitoreos Generales", icon: "table" } // NUEVA TABLA
];
...
document.getElementById('tabSelectorChecks').onclick = e => {
  if (e.target.type === 'checkbox') {
    ...
    if (e.target.checked) {
      ...
      else e.target.checked = true; // siempre debe haber al menos una
    }
