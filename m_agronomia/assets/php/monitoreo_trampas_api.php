...
try {
    ...
    if ($action==='inactivar'){
        ...
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st=$pg->prepare("UPDATE monitoreo_trampas SET error_registro='inactivo' WHERE monitoreo_trampas_id=?");
        $st->execute([$id]);
    ...
    if ($action === 'activar') {
        ...
        if(!$id) respond(['success'=>false,'error'=>'id_invalid'],400);
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st = $pg->prepare("UPDATE monitoreo_trampas SET error_registro = NULL WHERE monitoreo_trampas_id = :id");
        $st->execute(['id'=>$id]);
        $success = $st->rowCount() > 0;
