...
try {
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            'ct_cal_trampas_id','fecha','hora','responsable','colaborador','plantacion','finca','siembra','lote','parcela','trampa','plaga','hembra','macho','lado_a','lado_b','estado_lona','estado_trampa','estado_ubicacion','estado_ventana','estado_cania','estado_melaza','estado_feromona','calificación','observaciones','error_registro','supervision','check'
        ];
    ...
    if ($action==='inactivar'){
        ...
        try {
            ...
            $st=$pgMain->prepare("UPDATE ct_cal_trampas SET error_registro='inactivo' WHERE ct_cal_trampas_id=?");
            $st->execute([$id]);
        ...
        try {
            ...
            $stTemp=$pgTemp->prepare("UPDATE ct_cal_trampas SET error_registro='inactivo' WHERE ct_cal_trampas_id=?");
            $stTemp->execute([$id]);
