...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM ct_cal_trampas $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM ct_cal_trampas $whereSql";
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            ...
            'parcela','trampa','plaga','hembra','macho','lado_a','lado_b','estado_lona','estado_trampa','estado_ubicacion',
            'estado_ventana','estado_cania','estado_melaza','estado_feromona','calificacion','observaciones','supervision',
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.ct_cal_trampas SET supervision='pendiente', "check"=0 WHERE ct_cal_trampas_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
