...
/**
 * API de Control de Calidad de Trampas
 *
 * Gestiona registros de calidad de trampas, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM ct_cal_trampas $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM ct_cal_trampas $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        $id = $body['ct_cal_trampas_id'] ?? null;
        if ((!$id || trim($id) === '') && isset($body['id'])) $id = $body['id'];
        if (!$id || trim($id) === '') throw new RuntimeException('ct_cal_trampas_id requerido');
        ...
        $stC = $pg->prepare("SELECT 1 FROM ct_cal_trampas WHERE ct_cal_trampas_id=?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE ct_cal_trampas SET " . implode(', ', $updatePairs) . " WHERE ct_cal_trampas_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('ct_cal_trampas_id', $insertCols, true)) {
                $insertCols[] = 'ct_cal_trampas_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO ct_cal_trampas (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_CT_CAL_TRAMPAS' : 'CREATE_CT_CAL_TRAMPAS';
            $logDesc = $exists ? 'Actualización Calidad Trampas' : 'Registro Calidad Trampas';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE ct_cal_trampas SET error_registro = 'inactivo' WHERE ct_cal_trampas_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.ct_cal_trampas SET error_registro = 'inactivo' WHERE ct_cal_trampas_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_CT_CAL_TRAMPAS', 'Inactivación Calidad Trampas', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE ct_cal_trampas SET error_registro = NULL WHERE ct_cal_trampas_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.ct_cal_trampas SET error_registro = NULL WHERE ct_cal_trampas_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_CT_CAL_TRAMPAS', 'Reactivación Calidad Trampas', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.ct_cal_trampas SET supervision='rechazado', \"check\"=0 WHERE ct_cal_trampas_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.ct_cal_trampas SET supervision='rechazado', \"check\"=0 WHERE ct_cal_trampas_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.ct_cal_trampas WHERE ct_cal_trampas_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_CT_CAL_TRAMPAS', 'Rechazo Calidad Trampas', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.ct_cal_trampas SET supervision='aprobado', \"check\"=1 WHERE ct_cal_trampas_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.ct_cal_trampas WHERE ct_cal_trampas_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.ct_cal_trampas ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.ct_cal_trampas SET supervision='aprobado', \"check\"=1 WHERE ct_cal_trampas_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_CT_CAL_TRAMPAS', 'Aprobación Calidad Trampas', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
