 ...
 *
 * Emisión de eventos SSE (Server-Sent Events) para monitoreo en tiempo real.
 * 
 * Funcionalidad:
 *  - Emite eventos sobre la cantidad de tareas pendientes en diversas entidades.
 *  - Tipos de eventos: init (estado inicial), update (cambio detectado), ping (latido), error.
 *  - Intervalo de verificación: 5 segundos.
 *
 * Parámetros GET:
 *  - entidad (opcional): Filtra el monitoreo a una entidad específica.
 *
 * Requisitos:
 *  - Requiere autenticación y rol de administrador o auxiliar.
 */
...
{
  $base = ['mantenimientos', 'monitoreos_generales', 'oficios_varios_palma', 'cosecha_fruta', 'reco_fruta', 'fertilizacion_organica', 'aud_maquinaria', 'erradicaciones', 'compostaje'];
...
$pg = $pg_temporal;
if (!isset($pg)) {
  sse_send('error', ['message' => 'db_postgres.php no define $pg_temporal']);
  exit;
}
...
try {
  $estado = contar($pg, $ent ?: null);
  sse_send('init', $estado);
  $ultimo = (int)$estado['total'];
} catch (Throwable $e) {
