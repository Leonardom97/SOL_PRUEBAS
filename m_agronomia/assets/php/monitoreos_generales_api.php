...
/**
 * API de Monitoreos Generales
 *
 * Gestiona registros de monitoreos generales, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM monitoreos_generales $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM monitoreos_generales $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        $id = $body['monitoreos_generales_id'] ?? null;
        if ((!$id || trim($id) === '') && isset($body['id'])) $id = $body['id'];
        if (!$id || trim($id) === '') throw new RuntimeException('monitoreos_generales_id requerido');
        ...
        $stC = $pg->prepare("SELECT 1 FROM monitoreos_generales WHERE monitoreos_generales_id=?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE monitoreos_generales SET " . implode(', ', $updatePairs) . " WHERE monitoreos_generales_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('monitoreos_generales_id', $insertCols, true)) {
                $insertCols[] = 'monitoreos_generales_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO monitoreos_generales (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_MONITOREOS_GENERALES' : 'CREATE_MONITOREOS_GENERALES';
            $logDesc = $exists ? 'Actualización Monitoreos Generales' : 'Registro Monitoreos Generales';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE monitoreos_generales SET error_registro = 'inactivo' WHERE monitoreos_generales_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.monitoreos_generales SET error_registro = 'inactivo' WHERE monitoreos_generales_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_MONITOREOS_GENERALES', 'Inactivación Monitoreos Generales', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE monitoreos_generales SET error_registro = NULL WHERE monitoreos_generales_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.monitoreos_generales SET error_registro = NULL WHERE monitoreos_generales_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_MONITOREOS_GENERALES', 'Reactivación Monitoreos Generales', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.monitoreos_generales SET supervision='rechazado', \"check\"=0 WHERE monitoreos_generales_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.monitoreos_generales SET supervision='rechazado', \"check\"=0 WHERE monitoreos_generales_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.monitoreos_generales WHERE monitoreos_generales_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_MONITOREOS_GENERALES', 'Rechazo Monitoreos Generales', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.monitoreos_generales SET supervision='aprobado', \"check\"=1 WHERE monitoreos_generales_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.monitoreos_generales WHERE monitoreos_generales_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.monitoreos_generales ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.monitoreos_generales SET supervision='aprobado', \"check\"=1 WHERE monitoreos_generales_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_MONITOREOS_GENERALES', 'Aprobación Monitoreos Generales', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
