...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM ct_cal_labores $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM ct_cal_labores $whereSql";
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            'ct_cal_labores_id','fecha','hora','colaborador','tipo_auditoria','labor_especifica','labor','plantacion','finca','siembra','lote','parcela','linea','palma','linea_entrada','linea_salida','tipo_validacion','grado_enfermedad','numero_galerias','promedio','estado','producto','observacion','supervision','error_registro','check'
        ];
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.ct_cal_labores SET supervision='pendiente', "check"=0 WHERE ct_cal_labores_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
