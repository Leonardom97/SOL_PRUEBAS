...
/**
 * API de Auditoría de Maquinaria (aud_maquinaria)
 *
 * Gestiona las operaciones CRUD y de flujo de aprobación para los registros de maquinaria.
 * Soporta las siguientes acciones:
 *  - action=conexion (GET): Listado con filtros, paginación y ordenamiento
 *  - action=actualizar (POST JSON): Insertar o actualizar (Upsert) en tabla temporal
 *  - action=inactivar (POST JSON): Marca error_registro='inactivo' en tabla principal
 *  - action=rechazar (POST JSON): Rechaza el registro en principal/temporal (Requiere permisos de admin)
 *  - action=aprobar (POST JSON): Aprueba el registro moviéndolo a principal (Requiere permisos de admin)
 *
 * Sigue el patrón de diseño establecido en mantenimientos_api.php
 */
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM aud_maquinaria $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM aud_maquinaria $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        $id = $body['aud_maquinaria_id'] ?? null;
        if ((!$id || trim($id) === '') && isset($body['id'])) $id = $body['id'];
        if (!$id || trim($id) === '') throw new RuntimeException('aud_maquinaria_id requerido');
        ...
        $stC = $pg->prepare("SELECT 1 FROM aud_maquinaria WHERE aud_maquinaria_id = ?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE aud_maquinaria SET " . implode(', ', $updatePairs) . " WHERE aud_maquinaria_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('aud_maquinaria_id', $insertCols, true)) {
                $insertCols[] = 'aud_maquinaria_id';
                $placeholders[] = '?';
                $vals[] = $id;
            }
            $sql = "INSERT INTO aud_maquinaria (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $placeholders) . ")";
            $ok = $pg->prepare($sql)->execute($vals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_AUD_MAQUINARIA' : 'CREATE_AUD_MAQUINARIA';
            $logDesc = $exists ? 'Actualización Auditoría Maquinaria' : 'Registro Auditoría Maquinaria';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_maquinaria SET error_registro = 'inactivo' WHERE aud_maquinaria_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.aud_maquinaria SET error_registro = 'inactivo' WHERE aud_maquinaria_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_AUD_MAQUINARIA', 'Inactivación Auditoría Maquinaria', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_maquinaria SET error_registro = NULL WHERE aud_maquinaria_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_maquinaria SET error_registro = NULL WHERE aud_maquinaria_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_AUD_MAQUINARIA', 'Reactivación Auditoría Maquinaria', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $id = isset($body['aud_maquinaria_id']) ? trim($body['aud_maquinaria_id']) : '';
        if ($id === '') $id = isset($body['id']) ? trim($body['id']) : '';
        if ($id === '') throw new RuntimeException('aud_maquinaria_id requerido');
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_maquinaria SET supervision='rechazado', \"check\"=0 WHERE aud_maquinaria_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_maquinaria SET supervision='rechazado', \"check\"=0 WHERE aud_maquinaria_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.aud_maquinaria WHERE aud_maquinaria_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_AUD_MAQUINARIA', 'Rechazo Auditoría Maquinaria', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_maquinaria SET supervision='aprobado', \"check\"=1 WHERE aud_maquinaria_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.aud_maquinaria WHERE aud_maquinaria_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.aud_maquinaria ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_maquinaria SET supervision='aprobado', \"check\"=1 WHERE aud_maquinaria_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_AUD_MAQUINARIA', 'Aprobación Auditoría Maquinaria', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
