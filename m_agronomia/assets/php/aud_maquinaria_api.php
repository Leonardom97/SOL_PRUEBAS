 ...
 *
 * Patrón similar a mantenimientos_api.php
 */
...
try {
    ...
    $cols = [
        ...
        'tipo_auditoria','error_registro','supervision','check'
    ];
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM aud_maquinaria $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt = $pg->prepare($sql);
        ...
        $sqlT = "SELECT COUNT(*) FROM aud_maquinaria $whereSql";
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_maquinaria SET supervision='pendiente', "check"=0 WHERE aud_maquinaria_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
