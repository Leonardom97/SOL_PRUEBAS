...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM compactacion $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM compactacion $whereSql";
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            'compactacion_id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela','linea_compactacion','palma_compactacion','ubicacion_compactacion','presion','profundidad','supervision','error_registro','check'
        ];
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.compactacion SET supervision='pendiente', "check"=0 WHERE compactacion_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
