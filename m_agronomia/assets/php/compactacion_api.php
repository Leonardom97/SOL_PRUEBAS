...
try {
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            'compactacion_id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela','linea','palma','ubicacion','presion','profundidad','error_registro','supervision','check'
        ];
        ...
        if($exists){
            $sql="UPDATE mantenimientos SET ".implode(', ',$updatePairs)." WHERE compactacion_id = ?";
    ...
    if ($action==='inactivar'){
        ...
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st=$pg->prepare("UPDATE mantenimientos SET error_registro='inactivo' WHERE compactacion_id=?");
        $st->execute([$id]);
