 ...
 * API de Reco Fruta
 *
 * Gestiona las actividades de reco fruta, incluyendo:
        ...
        if ($exists) {
            ...
            if ($ok) {
                $logger->log(
                    ...
                    'UPDATE_RECO_FRUTA',
                    'Actualización Reco Fruta',
                    ['id' => $id]
                );
            }

            // (bloque duplicado mantenido pero con el mismo naming)
            if ($ok) {
                $logger->log(
                    $_SESSION['usuario_id'] ?? null,
                    $_SESSION['tipo_usuario'] ?? 'colaborador',
    ...
    if ($action === 'inactivar') {
        ...
        if ($ok) {
            $logger->log(
                $_SESSION['usuario_id'] ?? null,
                $_SESSION['tipo_usuario'] ?? 'colaborador',
                'INACTIVATE_RECO_FRUTA',
                'Inactivación Reco Fruta',
                ['id' => $id]
            );
        }
