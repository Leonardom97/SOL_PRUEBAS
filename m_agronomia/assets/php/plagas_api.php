...
/**
 * API de Control de Plagas
 *
 * Gestiona el registro y monitoreo de plagas, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM plagas $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM plagas $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        if ($ok) {
            $logAction = $exists ? 'UPDATE_PLAGAS' : 'CREATE_PLAGAS';
            $logDesc = $exists ? 'Actualización Plagas' : 'Registro Plagas';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE plagas SET error_registro = 'inactivo' WHERE plagas_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.plagas SET error_registro = 'inactivo' WHERE plagas_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_PLAGAS', 'Inactivación Plagas', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE plagas SET error_registro = NULL WHERE plagas_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.plagas SET error_registro = NULL WHERE plagas_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_PLAGAS', 'Reactivación Plagas', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_PLAGAS', 'Rechazo Plagas', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_PLAGAS', 'Aprobación Plagas', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
