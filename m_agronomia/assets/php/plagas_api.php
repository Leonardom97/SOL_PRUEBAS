...
/**
 * API plagas.
 * Acciones: list, upsert, aprobar, rechazar, inactivate
 * Alias entrada: conexion|listar|list, actualizar|upsert, aprobar|approve, rechazar|reject, inactivar|desactivar|inactivate
 *
 * Ajustes:
 * - Aceptar id genérico 'id' como fallback.
 * - RECHAZAR: intenta MAIN y TEMP; si MAIN afectó, elimina TEMP; devuelve detalles.
 * - Upsert responde "guardado correctamente" al éxito.
 */
...
$idCol='plagas_id';
$colsAllowed=[
  'plagas_id','fecha','hora','colaborador','plantacion','finca','siembra','lote',
  'parcela','linea','palma','ubicacion','orden','plaga','etapa','cantidad',
  'instar','estado','supervision','check','error_registro'
];
...
/* RECHAZAR (fallback temporal con eliminación si MAIN actualiza) */
if($action==='rechazar'){
  if($method!=='POST') respond(['success'=>false,'error'=>'method_not_allowed','allowed'=>'POST'],405);
  try{
    $id=isset($body[$idCol])?trim($body[$idCol]):'';
    ...
    respond(['success'=>($updatedMain+$updatedTemp)>0,'action'=>'rechazar','updated_main'=>$updatedMain,'updated_temp'=>$updatedTemp,'deleted_temp'=>$deletedTemp,'id'=>$id,'estado'=>'rechazado','warnings'=>$warnings]);
  }catch(Throwable $e){
    respond(['success'=>false,'error'=>'exception','message'=>$e->getMessage()]);
  }
}
