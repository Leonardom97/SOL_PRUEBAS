...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM coberturas $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM coberturas $whereSql";
    ...
    if ($action==='inactivar'){
        ...
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st=$pg->prepare("UPDATE coberturas SET error_registro='inactivo' WHERE coberturas_id=?");
        $st->execute([$id]);
    ...
    if ($action === 'activar') {
        ...
        if(!$id) respond(['success'=>false,'error'=>'id_invalid'],400);
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st = $pg->prepare("UPDATE coberturas SET error_registro = NULL WHERE coberturas_id = :id");
        $st->execute(['id'=>$id]);
        $success = $st->rowCount() > 0;
