...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM aud_cosecha $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM aud_cosecha $whereSql";
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            ...
            'palmas_sin_alistar','pepas_frescas_plato','pepas_frescas_fuera_plato','nuevo_colaborador','supervision','error_registro','check'
        ];
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_cosecha SET supervision='pendiente', "check"=0 WHERE aud_cosecha_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
