...
/**
 * API de Erradicaciones
 *
 * Gestiona actividades de erradicación en la plantación, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM erradicaciones $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM erradicaciones $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        if ($ok) {
            $logAction = $exists ? 'UPDATE_ERRADICACIONES' : 'CREATE_ERRADICACIONES';
            $logDesc = $exists ? 'Actualización Erradicaciones' : 'Registro Erradicaciones';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE erradicaciones SET error_registro = 'inactivo' WHERE erradicaciones_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.erradicaciones SET error_registro = 'inactivo' WHERE erradicaciones_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_ERRADICACIONES', 'Inactivación Erradicaciones', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE erradicaciones SET error_registro = NULL WHERE erradicaciones_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.erradicaciones SET error_registro = NULL WHERE erradicaciones_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_ERRADICACIONES', 'Reactivación Erradicaciones', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_ERRADICACIONES', 'Rechazo Erradicaciones', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_ERRADICACIONES', 'Aprobación Erradicaciones', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
