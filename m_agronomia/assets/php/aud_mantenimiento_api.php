<?php
/**
 * API aud_mantenimiento (adaptado).
...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM aud_mantenimiento $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM aud_mantenimiento $whereSql";
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_mantenimiento SET supervision='pendiente', "check"=0 WHERE aud_mantenimiento_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
