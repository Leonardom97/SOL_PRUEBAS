...
/**
 * API de Auditoría de Mantenimiento (aud_mantenimiento)
 *
 * Gestiona las operaciones CRUD y de flujo de aprobación para los registros de mantenimiento.
 ...
 *  - Acepta 'id' como fallback para 'aud_mantenimiento_id'.
 */
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM aud_mantenimiento $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM aud_mantenimiento $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        if (!$id || trim($id) === '') throw new RuntimeException('aud_mantenimiento_id requerido');
        ...
        $stC = $pg->prepare("SELECT 1 FROM aud_mantenimiento WHERE aud_mantenimiento_id=?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE aud_mantenimiento SET " . implode(', ', $updatePairs) . " WHERE aud_mantenimiento_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('aud_mantenimiento_id', $insertCols, true)) {
                $insertCols[] = 'aud_mantenimiento_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO aud_mantenimiento (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_AUD_MANTENIMIENTO' : 'CREATE_AUD_MANTENIMIENTO';
            $logDesc = $exists ? 'Actualización Auditoría Mantenimiento' : 'Registro Auditoría Mantenimiento';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_mantenimiento SET error_registro = 'inactivo' WHERE aud_mantenimiento_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.aud_mantenimiento SET error_registro = 'inactivo' WHERE aud_mantenimiento_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_AUD_MANTENIMIENTO', 'Inactivación Auditoría Mantenimiento', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_mantenimiento SET error_registro = NULL WHERE aud_mantenimiento_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_mantenimiento SET error_registro = NULL WHERE aud_mantenimiento_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_AUD_MANTENIMIENTO', 'Reactivación Auditoría Mantenimiento', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $id = isset($body['aud_mantenimiento_id']) ? trim($body['aud_mantenimiento_id']) : '';
        if ($id === '') $id = isset($body['id']) ? trim($body['id']) : '';
        if ($id === '') throw new RuntimeException('aud_mantenimiento_id requerido');
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_mantenimiento SET supervision='rechazado', \"check\"=0 WHERE aud_mantenimiento_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_mantenimiento SET supervision='rechazado', \"check\"=0 WHERE aud_mantenimiento_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.aud_mantenimiento WHERE aud_mantenimiento_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_AUD_MANTENIMIENTO', 'Rechazo Auditoría Mantenimiento', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_mantenimiento SET supervision='aprobado', \"check\"=1 WHERE aud_mantenimiento_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.aud_mantenimiento WHERE aud_mantenimiento_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.aud_mantenimiento ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_mantenimiento SET supervision='aprobado', \"check\"=1 WHERE aud_mantenimiento_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_AUD_MANTENIMIENTO', 'Aprobación Auditoría Mantenimiento', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
