<?php
/**
 * API ct_polinizacion_flores (adaptado).
...
try {
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            ...
            'flores_nuevas_sin_polinizar','aplicacion_2','aplicacion_3','labor','supervision','error_registro','check'
        ];
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stCheck = $pgMain->prepare("SELECT 1 FROM ct_polinizacion_flores WHERE ct_polinizacion_flores_id = :id LIMIT 1");
        ...
        try {
            ...
            // First check if record exists
            $stCheck = $pgTemp->prepare("SELECT 1 FROM ct_polinizacion_flores WHERE ct_polinizacion_flores_id = :id LIMIT 1");
            $stCheck->execute(['id'=>$id]);
            $foundTemp = (bool)$stCheck->fetchColumn();
            
            if ($foundTemp) {
                $stTemp = $pgTemp->prepare("UPDATE ct_polinizacion_flores SET error_registro = 'inactivo' WHERE ct_polinizacion_flores_id = :id");
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            if ($foundTemp) {
                $stTemp = $pgTemp->prepare("UPDATE ct_polinizacion_flores SET error_registro = NULL WHERE ct_polinizacion_flores_id = :id");
                $stTemp->execute(['id'=>$id]);
                $updatedTemp = $stTemp->rowCount();
            }
        } catch(Throwable $e) {
