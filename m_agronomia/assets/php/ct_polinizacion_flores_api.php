...
try {
    ...
    if ($action==='inactivar'){
        ...
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st=$pg->prepare("UPDATE ct_polinizacion_flores SET error_registro='inactivo' WHERE ct_polinizacion_flores_id=?");
        $st->execute([$id]);
    ...
    if ($action === 'activar') {
        ...
        if(!$id) respond(['success'=>false,'error'=>'id_invalid'],400);
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st = $pg->prepare("UPDATE ct_polinizacion_flores SET error_registro = NULL WHERE ct_polinizacion_flores_id = :id");
        $st->execute(['id'=>$id]);
        $success = $st->rowCount() > 0;
