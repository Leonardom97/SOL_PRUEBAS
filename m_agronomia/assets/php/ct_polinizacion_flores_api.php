...
/**
 * API de Control de Polinización de Flores
 *
 * Gestiona registros de polinización de flores, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM ct_polinizacion_flores $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM ct_polinizacion_flores $whereSql";
    ...
    if ($action === 'actualizar') {
        ...
        $cols = [
            ...
            'flores_buenas',
            'flores_ayudadas',
            'flores_anteriores_sin_polinizar',
            'flores_nuevas_sin_polinizar',
            'aplicacion_2',
            'aplicacion_3',
            'labor',
            'supervision',
        ...
        if (!$id || trim($id) === '') throw new RuntimeException('ct_polinizacion_flores_id requerido');
        ...
        foreach ($cols as $c) {
            if (array_key_exists($c, $body)) {
                ...
                $insertPlaceholders[] = '?';
                $insertVals[] = $body[$c];
                if ($c !== 'ct_polinizacion_flores_id') {
                    $updatePairs[] = "\"$c\" = ?";
                    $updateVals[] = $body[$c];
                }
            }
        ...
        $stC = $pg->prepare("SELECT 1 FROM ct_polinizacion_flores WHERE ct_polinizacion_flores_id=?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE ct_polinizacion_flores SET " . implode(', ', $updatePairs) . " WHERE ct_polinizacion_flores_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('ct_polinizacion_flores_id', $insertCols, true)) {
                $insertCols[] = 'ct_polinizacion_flores_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO ct_polinizacion_flores (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_CT_POLINIZACION_FLORES' : 'CREATE_CT_POLINIZACION_FLORES';
            $logDesc = $exists ? 'Actualización Polinización Flores' : 'Registro Polinización Flores';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE ct_polinizacion_flores SET error_registro = 'inactivo' WHERE ct_polinizacion_flores_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.ct_polinizacion_flores SET error_registro = 'inactivo' WHERE ct_polinizacion_flores_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_CT_POLINIZACION_FLORES', 'Inactivación Polinización Flores', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE ct_polinizacion_flores SET error_registro = NULL WHERE ct_polinizacion_flores_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.ct_polinizacion_flores SET error_registro = NULL WHERE ct_polinizacion_flores_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_CT_POLINIZACION_FLORES', 'Reactivación Polinización Flores', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.ct_polinizacion_flores SET supervision='rechazado', \"check\"=0 WHERE ct_polinizacion_flores_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.ct_polinizacion_flores SET supervision='rechazado', \"check\"=0 WHERE ct_polinizacion_flores_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.ct_polinizacion_flores WHERE ct_polinizacion_flores_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_CT_POLINIZACION_FLORES', 'Rechazo Polinización Flores', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.ct_polinizacion_flores SET supervision='aprobado', \"check\"=1 WHERE ct_polinizacion_flores_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.ct_polinizacion_flores WHERE ct_polinizacion_flores_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.ct_polinizacion_flores ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.ct_polinizacion_flores SET supervision='aprobado', \"check\"=1 WHERE ct_polinizacion_flores_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_CT_POLINIZACION_FLORES', 'Aprobación Polinización Flores', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
