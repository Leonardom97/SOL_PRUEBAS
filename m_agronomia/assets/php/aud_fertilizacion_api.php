<?php
/**
 * API aud_fertilizacion (adaptado).
...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM aud_fertilizacion $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_fertilizacion SET supervision='pendiente', "check"=0 WHERE aud_fertilizacion_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
