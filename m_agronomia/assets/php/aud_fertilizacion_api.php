...
/**
 * API de Auditoría de Fertilización (aud_fertilizacion)
 *
 * Gestiona las operaciones CRUD y de flujo de aprobación para los registros de labor de fertilización.
 * Soporta las siguientes acciones:
 *  - Upsert (actualizar): Guarda en la BD temporal.
 *  - Rechazar: Intenta en tabla principal, actualiza temporal siempre; elimina de temporal si fue rechazado en principal.
 *  - Aprobar: Intenta actualizar en principal; si no existe, inserta desde temporal; actualiza temporal siempre.
 *    Si la operación en principal es exitosa, elimina el registro temporal para limpiar pendientes.
 *  - Acepta 'id' como fallback para 'aud_fertilizacion_id'.
 */
...
/**
 * Mapear acciones de entrada (incluso en inglés) a acciones estándar en español
 * 
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM aud_fertilizacion $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlT = "SELECT COUNT(*) FROM aud_fertilizacion $whereSql";
    ...
    if ($action === 'actualizar') {
        ...
        if (!$id || trim($id) === '') throw new RuntimeException('aud_fertilizacion_id requerido');
        ...
        $stC = $pg->prepare("SELECT 1 FROM aud_fertilizacion WHERE aud_fertilizacion_id=?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE aud_fertilizacion SET " . implode(', ', $updatePairs) . " WHERE aud_fertilizacion_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('aud_fertilizacion_id', $insertCols, true)) {
                $insertCols[] = 'aud_fertilizacion_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO aud_fertilizacion (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_AUD_FERTILIZACION' : 'CREATE_AUD_FERTILIZACION';
            $logDesc = $exists ? 'Actualización Auditoría Fertilización' : 'Registro Auditoría Fertilización';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_fertilizacion SET error_registro = 'inactivo' WHERE aud_fertilizacion_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.aud_fertilizacion SET error_registro = 'inactivo' WHERE aud_fertilizacion_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_AUD_FERTILIZACION', 'Inactivación Auditoría Fertilización', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_fertilizacion SET error_registro = NULL WHERE aud_fertilizacion_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_fertilizacion SET error_registro = NULL WHERE aud_fertilizacion_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_AUD_FERTILIZACION', 'Reactivación Auditoría Fertilización', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $id = isset($body['aud_fertilizacion_id']) ? trim($body['aud_fertilizacion_id']) : '';
        if ($id === '') $id = isset($body['id']) ? trim($body['id']) : '';
        if ($id === '') throw new RuntimeException('aud_fertilizacion_id requerido');
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_fertilizacion SET supervision='rechazado', \"check\"=0 WHERE aud_fertilizacion_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_fertilizacion SET supervision='rechazado', \"check\"=0 WHERE aud_fertilizacion_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.aud_fertilizacion WHERE aud_fertilizacion_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_AUD_FERTILIZACION', 'Rechazo Auditoría Fertilización', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_fertilizacion SET supervision='aprobado', \"check\"=1 WHERE aud_fertilizacion_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.aud_fertilizacion WHERE aud_fertilizacion_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.aud_fertilizacion ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_fertilizacion SET supervision='aprobado', \"check\"=1 WHERE aud_fertilizacion_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_AUD_FERTILIZACION', 'Aprobación Auditoría Fertilización', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
