 ...
 *
 * Gestiona las actividades de mantenimiento, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        if (!empty($_GET['ordenColumna'])) {
            ...
            $ordenAsc = (!isset($_GET['ordenAsc']) || $_GET['ordenAsc'] == '1') ? 'ASC' : 'DESC';
            $orderSql = " ORDER BY \"$ordenColumna\" $ordenAsc";
        }

        $sql = "SELECT * FROM mantenimientos $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
    ...
    if ($action === 'actualizar') {
        ...
        if ($ok) {
            $logAction = $exists ? 'UPDATE_MANTENIMIENTOS' : 'CREATE_MANTENIMIENTOS';
            $logDesc = $exists ? 'Actualización Mantenimientos' : 'Registro Mantenimientos';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE mantenimientos SET error_registro = 'inactivo' WHERE mantenimientos_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.mantenimientos SET error_registro = 'inactivo' WHERE mantenimientos_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_MANTENIMIENTOS', 'Inactivación Mantenimientos', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE mantenimientos SET error_registro = NULL WHERE mantenimientos_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.mantenimientos SET error_registro = NULL WHERE mantenimientos_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_MANTENIMIENTOS', 'Reactivación Mantenimientos', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $id = isset($body['mantenimientos_id']) ? trim($body['mantenimientos_id']) : '';
        if ($id === '') $id = isset($body['id']) ? trim($body['id']) : '';
        if ($id === '') throw new RuntimeException('mantenimientos_id requerido');
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.mantenimientos SET supervision='rechazado', \"check\"=0 WHERE mantenimientos_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.mantenimientos SET supervision='rechazado', \"check\"=0 WHERE mantenimientos_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.mantenimientos WHERE mantenimientos_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_MANTENIMIENTOS', 'Rechazo Mantenimientos', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.mantenimientos SET supervision='aprobado', \"check\"=1 WHERE mantenimientos_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.mantenimientos WHERE mantenimientos_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.mantenimientos ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.mantenimientos SET supervision='aprobado', \"check\"=1 WHERE mantenimientos_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_MANTENIMIENTOS', 'Aprobación Mantenimientos', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
