...
try {
    ...
    if ($action==='inactivar'){
        ...
        $ok=$st->execute([$id]);
        $respond(['success'=>$ok && $st->rowCount()>0,'action'=>'inactivar','id'=>$id,'estado'=>'inactivo']);
    }
    ...
    if ($action==='aprobar'){
        throw new RuntimeException('acción aprobar sin implementar en este endpoint (mantener implementación actual si la tienes)');
    }
