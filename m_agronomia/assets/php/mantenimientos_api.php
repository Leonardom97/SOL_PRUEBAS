...
try {
    ...
    if ($action === 'revertir') {
        ...
        // Revertir en MAIN: cambiar supervision a 'pendiente' o similar
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.mantenimientos SET supervision='pendiente', \"check\"=0 WHERE mantenimientos_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
