...
try {
    ...
    if ($action==='actualizar') {
        ...
        if($exists){
            $sql="UPDATE mantenimientos SET ".implode(', ',$updatePairs)." WHERE reporte_lote_monitoreo_id = ?";
    ...
    if ($action==='inactivar'){
        ...
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st=$pg->prepare("UPDATE mantenimientos SET error_registro='inactivo' WHERE reporte_lote_monitoreo_id=?");
        $st->execute([$id]);
