...
/**
 * API de Reporte de Lote de Monitoreo
 *
 * Gestiona registros de reportes de lote de monitoreo, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM reporte_lote_monitoreo $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM reporte_lote_monitoreo $whereSql";
