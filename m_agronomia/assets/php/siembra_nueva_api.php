...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM siembra_nueva $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM siembra_nueva $whereSql";
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.siembra_nueva SET supervision='pendiente', "check"=0 WHERE siembra_nueva_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
