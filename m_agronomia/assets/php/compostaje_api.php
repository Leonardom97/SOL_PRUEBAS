 ...
 *
 * Sigue el patrón de mantenimientos_api.php / aud_maquinaria_api.php
 */
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM compostaje $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt = $pg->prepare($sql);
        ...
        $sqlT = "SELECT COUNT(*) FROM compostaje $whereSql";
    ...
    if ($action === 'inactivar') {
        ...
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st = $pg->prepare("UPDATE compostaje SET error_registro='inactivo' WHERE compostaje_id = ?");
        $st->execute([$id]);
    ...
    if ($action === 'activar') {
        ...
        if(!$id) respond(['success'=>false,'error'=>'id_invalid'],400);
        $pg->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $st = $pg->prepare("UPDATE compostaje SET error_registro = NULL WHERE compostaje_id = :id");
        $st->execute(['id'=>$id]);
        $success = $st->rowCount() > 0;
