...
/**
 * API de Auditoría de Pérdidas (aud_perdidas)
 *
 * Gestiona las operaciones CRUD y de flujo de aprobación para los registros de pérdidas.
 ...
 *  - Acepta 'id' como fallback para 'aud_perdidas_id'.
 */
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM aud_perdidas $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM aud_perdidas $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        if (!$id || trim($id) === '') throw new RuntimeException('aud_perdidas_id requerido');
        ...
        $stC = $pg->prepare("SELECT 1 FROM aud_perdidas WHERE aud_perdidas_id=?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE aud_perdidas SET " . implode(', ', $updatePairs) . " WHERE aud_perdidas_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('aud_perdidas_id', $insertCols, true)) {
                $insertCols[] = 'aud_perdidas_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO aud_perdidas (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_AUD_PERDIDAS' : 'CREATE_AUD_PERDIDAS';
            $logDesc = $exists ? 'Actualización Auditoría Pérdidas' : 'Registro Auditoría Pérdidas';
    ...
    if ($action === 'inactivar') {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_AUD_PERDIDAS', 'Inactivación Auditoría Pérdidas', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_AUD_PERDIDAS', 'Reactivación Auditoría Pérdidas', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $id = isset($body['aud_perdidas_id']) ? trim($body['aud_perdidas_id']) : '';
        if ($id === '') $id = isset($body['id']) ? trim($body['id']) : '';
        if ($id === '') throw new RuntimeException('aud_perdidas_id requerido');
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_perdidas SET supervision='rechazado', \"check\"=0 WHERE aud_perdidas_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_perdidas SET supervision='rechazado', \"check\"=0 WHERE aud_perdidas_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.aud_perdidas WHERE aud_perdidas_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_AUD_PERDIDAS', 'Rechazo Auditoría Pérdidas', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_perdidas SET supervision='aprobado', \"check\"=1 WHERE aud_perdidas_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.aud_perdidas WHERE aud_perdidas_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.aud_perdidas ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_perdidas SET supervision='aprobado', \"check\"=1 WHERE aud_perdidas_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_AUD_PERDIDAS', 'Aprobación Auditoría Pérdidas', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
