...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM aud_perdidas $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM aud_perdidas $whereSql";
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            ...
            'siembra','finca','lote','parcela','linea','palma','fruto_suelto_perdido','racimos_perdidos','supervision','error_registro','check'
        ];
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_perdidas SET supervision='pendiente', "check"=0 WHERE aud_perdidas_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
