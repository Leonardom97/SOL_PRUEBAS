...
try {
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            'nivel_freatico_id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela','linea','palma','n_pozo_observacion','superficie_tubo','profundidad_agua','verificacion','validacion','enterrado','nivel_freatico','supervision','error_registro','check'
        ];
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.nivel_freatico SET supervision='pendiente', "check"=0 WHERE nivel_freatico_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
