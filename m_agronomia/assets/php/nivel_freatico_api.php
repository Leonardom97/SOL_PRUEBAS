...
/**
 * API de Nivel Freático
 *
 * Gestiona registros de nivel freático, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM nivel_freatico $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM nivel_freatico $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        if ($exists) {
            $sql = "UPDATE nivel_freatico SET " . implode(', ', $updatePairs) . " WHERE nivel_freatico_id = ?";
        ...
        if ($ok) {
            $logAction = $exists ? 'UPDATE_NIVEL_FREATICO' : 'CREATE_NIVEL_FREATICO';
            $logDesc = $exists ? 'Actualización Nivel Freatico' : 'Registro Nivel Freatico';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE nivel_freatico SET error_registro = 'inactivo' WHERE nivel_freatico_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.nivel_freatico SET error_registro = 'inactivo' WHERE nivel_freatico_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_NIVEL_FREATICO', 'Inactivación Nivel Freatico', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE nivel_freatico SET error_registro = NULL WHERE nivel_freatico_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.nivel_freatico SET error_registro = NULL WHERE nivel_freatico_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_NIVEL_FREATICO', 'Reactivación Nivel Freatico', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_NIVEL_FREATICO', 'Rechazo Nivel Freatico', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_NIVEL_FREATICO', 'Aprobación Nivel Freatico', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
