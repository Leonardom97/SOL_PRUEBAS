...
/**
 * API de Control de Calidad de Sanidad
 *
 * Gestiona registros de calidad de sanidad, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM ct_cal_sanidad $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM ct_cal_sanidad $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        if ($ok) {
            $logAction = $exists ? 'UPDATE_CT_CAL_SANIDAD' : 'CREATE_CT_CAL_SANIDAD';
            $logDesc = $exists ? 'Actualización Calidad Sanidad' : 'Registro Calidad Sanidad';
    ...
    if ($action === 'inactivar') {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_CT_CAL_SANIDAD', 'Inactivación Calidad Sanidad', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_CT_CAL_SANIDAD', 'Reactivación Calidad Sanidad', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_CT_CAL_SANIDAD', 'Rechazo Calidad Sanidad', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_CT_CAL_SANIDAD', 'Aprobación Calidad Sanidad', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
