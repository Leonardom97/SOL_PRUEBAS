...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM labores_diarias $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM labores_diarias $whereSql";
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            ...
            'horometro_final','hora_inicio','hora_final','cant_colaborador','labor_especifica','supervision','error_registro','check'
        ];
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.labores_diarias SET supervision='pendiente', "check"=0 WHERE labores_diarias_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
