...
/**
 * API de Oficios Varios Palma
 *
 * Gestiona registros de oficios varios en palma, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM oficios_varios_palma $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
    ...
    if ($action === 'actualizar') {
        ...
        if ($exists) {
            $sql = "UPDATE oficios_varios_palma SET " . implode(', ', $updatePairs) . " WHERE oficios_varios_palma_id = ?";
        ...
        if ($ok) {
            $logAction = $exists ? 'UPDATE_OFICIOS_VARIOS_PALMA' : 'CREATE_OFICIOS_VARIOS_PALMA';
            $logDesc = $exists ? 'Actualización Oficios Varios Palma' : 'Registro Oficios Varios Palma';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE oficios_varios_palma SET error_registro = 'inactivo' WHERE oficios_varios_palma_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.oficios_varios_palma SET error_registro = 'inactivo' WHERE oficios_varios_palma_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_OFICIOS_VARIOS_PALMA', 'Inactivación Oficios Varios Palma', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE oficios_varios_palma SET error_registro = NULL WHERE oficios_varios_palma_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.oficios_varios_palma SET error_registro = NULL WHERE oficios_varios_palma_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_OFICIOS_VARIOS_PALMA', 'Reactivación Oficios Varios Palma', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_OFICIOS_VARIOS_PALMA', 'Rechazo Oficios Varios Palma', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_OFICIOS_VARIOS_PALMA', 'Aprobación Oficios Varios Palma', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
