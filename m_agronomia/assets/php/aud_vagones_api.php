...
/**
 * API de Auditoría de Vagones (aud_vagones)
 *
 * Gestiona las operaciones CRUD y de flujo de aprobación para los registros de vagones.
 ...
 *  - Acepta 'id' como fallback para 'aud_vagones_id'.
 */
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM aud_vagones $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM aud_vagones $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        $cols = [
            ...
            'tipo',
            'numero_remision',
            'racimos_verdes_vagon',
            'racimos_sobremaduro_vagon',
            'racimos_enfermos_vagon',
            'racimos_tusas_vagon',
            'racimos_pedunculo_largo_vagon',
            'total_racimos',
            'racimos_auditados',
            'tipo_1',
            'tipo_2',
        ...
        if (!$id || trim($id) === '') throw new RuntimeException('aud_vagones_id requerido');
        ...
        $stC = $pg->prepare("SELECT 1 FROM aud_vagones WHERE aud_vagones_id=?");
        $stC->execute([$id]);
        $exists = (bool)$stC->fetchColumn();

        if ($exists) {
            $sql = "UPDATE aud_vagones SET " . implode(', ', $updatePairs) . " WHERE aud_vagones_id = ?";
            $valsToExecute = array_merge($updateVals, [$id]);
            $ok = $pg->prepare($sql)->execute($valsToExecute);
        } else {
            if (!in_array('aud_vagones_id', $insertCols, true)) {
                $insertCols[] = 'aud_vagones_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO aud_vagones (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_AUD_VAGONES' : 'CREATE_AUD_VAGONES';
            $logDesc = $exists ? 'Actualización Auditoría Vagones' : 'Registro Auditoría Vagones';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_vagones SET error_registro = 'inactivo' WHERE aud_vagones_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.aud_vagones SET error_registro = 'inactivo' WHERE aud_vagones_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_AUD_VAGONES', 'Inactivación Auditoría Vagones', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE aud_vagones SET error_registro = NULL WHERE aud_vagones_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_vagones SET error_registro = NULL WHERE aud_vagones_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_AUD_VAGONES', 'Reactivación Auditoría Vagones', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $id = isset($body['aud_vagones_id']) ? trim($body['aud_vagones_id']) : '';
        if ($id === '') $id = isset($body['id']) ? trim($body['id']) : '';
        if ($id === '') throw new RuntimeException('aud_vagones_id requerido');
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_vagones SET supervision='rechazado', \"check\"=0 WHERE aud_vagones_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_vagones SET supervision='rechazado', \"check\"=0 WHERE aud_vagones_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.aud_vagones WHERE aud_vagones_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_AUD_VAGONES', 'Rechazo Auditoría Vagones', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_vagones SET supervision='aprobado', \"check\"=1 WHERE aud_vagones_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.aud_vagones WHERE aud_vagones_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.aud_vagones ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.aud_vagones SET supervision='aprobado', \"check\"=1 WHERE aud_vagones_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_AUD_VAGONES', 'Aprobación Auditoría Vagones', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
