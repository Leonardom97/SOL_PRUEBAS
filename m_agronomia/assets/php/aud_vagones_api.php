...
try {
    ...
    if ($action==='conexion') {
        ...
        $sql="SELECT * FROM aud_vagones $whereSql $orderSql LIMIT :lim OFFSET :off";
        $stmt=$pg->prepare($sql);
        ...
        $sqlT="SELECT COUNT(*) FROM aud_vagones $whereSql";
    ...
    if ($action==='actualizar') {
        ...
        $cols=[
            ...
            'tipo_1','tipo_2','tipo_3','tipo_4','supervision','error_registro','check'
        ];
    ...
    if ($action === 'revertir') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.aud_vagones SET supervision='pendiente', "check"=0 WHERE aud_vagones_id=:id AND supervision='aprobado'");
            $stMain->execute(['id'=>$id]);
            $updatedMain = $stMain->rowCount();
        } catch(Throwable $e){
            $warnings[] = 'main_error: '.$e->getMessage();
