...
/**
 * API de Fertilización Orgánica
 *
 * Gestiona las actividades de fertilización orgánica, incluyendo:
...
try {
    ...
    if ($action === 'conexion') {
        ...
        $sql = "SELECT * FROM fertilizacion_organica $whereSql $orderSql LIMIT ? OFFSET ?";
        $stmt = $pg->prepare($sql);
        $i = 1;
        foreach ($params as $p) {
            $stmt->bindValue($i++, $p);
        ...
        $sqlCount = "SELECT COUNT(*) FROM fertilizacion_organica $whereSql";
    ...
    if ($action === 'actualizar' || $action === 'upsert') {
        ...
        $cols = [
            ...
            'dosis_kg',
            'unidad_aplicacion',
            'contratista_colaborador',
            'n_colaboradores',
            'colaboradores',
            'tipo_labor',
            'contratista_maquinaria',
            'n_operadores',
            'tipo_maquina',
            'nombre_operadores',
            'bultos_aplicados',
        ...
        if (!$id || trim($id) === '') throw new RuntimeException('fertilizacion_organica_id requerido');
        ...
        foreach ($cols as $c) {
            if (array_key_exists($c, $body)) {
                ...
                $insertPlaceholders[] = '?';
                $insertVals[] = $body[$c];
                if ($c !== 'fertilizacion_organica_id') {
                    $updatePairs[] = "\"$c\" = ?";
                    $updateVals[] = $body[$c];
                }
            }
        ...
        $stC = $pg->prepare("SELECT 1 FROM fertilizacion_organica WHERE fertilizacion_organica_id=?");
        $stC->execute([$id]);
        ...
        if ($exists) {
            ...
            if (!in_array('fertilizacion_organica_id', $insertCols, true)) {
                $insertCols[] = 'fertilizacion_organica_id';
                $insertPlaceholders[] = '?';
                $insertVals[] = $id;
            }
            $sql = "INSERT INTO fertilizacion_organica (" . implode(',', $insertCols) . ") VALUES (" . implode(',', $insertPlaceholders) . ")";
            $ok = $pg->prepare($sql)->execute($insertVals);
        }

        if ($ok) {
            $logAction = $exists ? 'UPDATE_FERTILIZACION_ORGANICA' : 'CREATE_FERTILIZACION_ORGANICA';
            $logDesc = $exists ? 'Actualización Fertilización Orgánica' : 'Registro Fertilización Orgánica';
    ...
    if ($action === 'inactivar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE fertilizacion_organica SET error_registro = 'inactivo' WHERE fertilizacion_organica_id = :id");
        ...
        try {
            ...
            $stTemp = $pgTemp->prepare("UPDATE public.fertilizacion_organica SET error_registro = 'inactivo' WHERE fertilizacion_organica_id = :id");
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'INACTIVATE_FERTILIZACION_ORGANICA', 'Inactivación Fertilización Orgánica', ['id' => $id]);
        }
    ...
    if ($action === 'activar') {
        ...
        try {
            ...
            $stMain = $pgMain->prepare("UPDATE fertilizacion_organica SET error_registro = NULL WHERE fertilizacion_organica_id = :id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.fertilizacion_organica SET error_registro = NULL WHERE fertilizacion_organica_id = :id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'colaborador', 'ACTIVATE_FERTILIZACION_ORGANICA', 'Reactivación Fertilización Orgánica', ['id' => $id]);
        }
    ...
    if ($action === 'rechazar') {
        ...
        $id = isset($body['fertilizacion_organica_id']) ? trim($body['fertilizacion_organica_id']) : '';
        if ($id === '') $id = isset($body['id']) ? trim($body['id']) : '';
        if ($id === '') throw new RuntimeException('fertilizacion_organica_id requerido');
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.fertilizacion_organica SET supervision='rechazado', \"check\"=0 WHERE fertilizacion_organica_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'main_error: ' . $e->getMessage();
        ...
        try {
            $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.fertilizacion_organica SET supervision='rechazado', \"check\"=0 WHERE fertilizacion_organica_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
            $warnings[] = 'temp_error: ' . $e->getMessage();
        ...
        if ($updatedMain > 0) {
            try {
                ...
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $del = $pgTemp->prepare("DELETE FROM public.fertilizacion_organica WHERE fertilizacion_organica_id = :id");
                $del->execute(['id' => $id]);
        ...
        $ok = ($updatedMain + $updatedTemp) > 0;
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'REJECT_FERTILIZACION_ORGANICA', 'Rechazo Fertilización Orgánica', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'rechazar',
    ...
    if ($action === 'aprobar') {
        ...
        try {
            $pgMain = getMain();
            $pgMain->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stMain = $pgMain->prepare("UPDATE public.fertilizacion_organica SET supervision='aprobado', \"check\"=1 WHERE fertilizacion_organica_id=:id");
            $stMain->execute(['id' => $id]);
            $updatedMain = $stMain->rowCount();
        } catch (Throwable $e) {
        ...
        if ($updatedMain == 0) {
            try {
                $pgTemp = getTemporal();
                $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stFetch = $pgTemp->prepare("SELECT * FROM public.fertilizacion_organica WHERE fertilizacion_organica_id = :id LIMIT 1");
                ...
                if ($row) {
                    ...
                    $sqlIns = "INSERT INTO public.fertilizacion_organica ($colsSql) VALUES ($placeSql)";
        ...
        try {
            if (!isset($pgTemp)) $pgTemp = getTemporal();
            $pgTemp->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stTemp = $pgTemp->prepare("UPDATE public.fertilizacion_organica SET supervision='aprobado', \"check\"=1 WHERE fertilizacion_organica_id=:id");
            $stTemp->execute(['id' => $id]);
            $updatedTemp = $stTemp->rowCount();
        } catch (Throwable $e) {
        ...
        if ($ok) {
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'admin', 'APPROVE_FERTILIZACION_ORGANICA', 'Aprobación Fertilización Orgánica', ['id' => $id]);
        }
        respond([
            'success' => $ok,
            'action' => 'aprobar',
