 ...
 * Alias: approve->aprobar, reject->rechazar.
 *
 * Ajustes:
 * - Acepta 'id' genérico como fallback en todas las acciones que requieren ID.
 * - Rechazar: ahora sigue el mismo flujo que cosecha_fruta.
 */
...
try{
  ...
  // Columnas que permitimos upsert (según tu HTML).
  $colsAllowed=[
  ...
  /* RECHAZAR (nuevo flujo, igual que cosecha_fruta) */
  if($action==='rechazar'){
    if($method!=='POST') respond(['success'=>false,'error'=>'method_not_allowed','allowed'=>'POST'],405);
    ...
    // 2) intentar actualizar en TEMP siempre
    try {
      $pgTemp = getTemporal();
