 ...
 * - Mantener EXACTAMENTE el comportamiento de guardado/upsert en la base TEMPORAL
 *   (para que "guardar" siga funcionando como antes).
 * - RECHAZAR mejorado (intenta MAIN y TEMP, elimina en TEMP si MAIN afectó).
 */
...
try{
  ...
  $table='cosecha_fruta'; $idCol='cosecha_fruta_id';
  $colsAllowed=[
  ...
  if($action==='upsert'){
    // Guardado/edición en la BD TEMPORAL (workflow original) - respuesta mínima.
  ...
  if($action==='inactivate'){
    ...
    $ok=$st->execute([$id]);
    respond(['success'=>$ok&&$st->rowCount()>0,'action'=>'inactivate','id'=>$id,'estado'=>'inactivo','updated'=>$st->rowCount()]);
  }

  if($action==='rechazar'){
    if($method!=='POST') respond(['success'=>false,'error'=>'method_not_allowed','allowed'=>'POST'],405);

    // Aceptar tanto cosecha_fruta_id como id (fallback)
    $id=isset($body[$idCol])?trim($body[$idCol]):'';
    if($id==='') $id = isset($body['id'])?trim($body['id']):'';
    ...
    // 1) intentar actualizar en MAIN (base principal)
    try {
      $pgMain = getMain();
    ...
    // 2) intentar actualizar en TEMP (base temporal) siempre (no interfiere con upsert)
    try {
      $pgTemp = getTemporal();
    ...
    // 3) si MAIN actualizó filas, intentar ELIMINAR la fila de la TEMPORAL
    if($updatedMain > 0){
      try{
