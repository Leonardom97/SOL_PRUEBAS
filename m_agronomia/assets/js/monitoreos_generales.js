...
(function () {
  ...
  // ---------------------------------------------------------------
  const DOM = {
    tbody: 'tbody-monitoreos-generales',
    table: 'tabla-monitoreos-generales',
    pagination: 'pagination-monitoreos-generales',
    exportBtn: 'exportBtnMonitoreos',
    clearBtn: 'clearFiltersBtnMonitoreos',
    limitSelect: 'limitSelectMonitoreos',
    selectAll: 'selectAllMonitoreos',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'monitoreos_generales_id', 'fecha', 'hora', 'colaborador', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'linea',
    'palma', 'grupo', 'estado', 'validacion', 'sintoma', 'observacion', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/monitoreos_generales_api.php';
  const ID_KEY = 'monitoreos_generales_id';
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        // debug: mostrar url de request en consola
        console.log('[monitoreos_generales] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch monitoreos_generales' + ' => ' + last);
  }
  function render() {
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[monitoreos_generales] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreos_generales] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreos_generales] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[monitoreos_generales] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[monitoreos_generales] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreos_generales] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreos_generales] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreos_generales] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreos_generales] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function exportar(t) {
    if (t === 'todo') {
      ...
      fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'monitoreos_generales_todo')); return;
    }
    let rows = data;
    if (t === 'seleccion') {
      rows = []; document.querySelectorAll(`#${DOM.tbody} .row-check`).forEach(ch => { if (ch.checked) rows.push(data[+ch.dataset.index]); });
    }
    buildXLSX(rows, 'monitoreos_generales_' + t);
  }
  // Estilo menú exportación restaurado
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuMonitoreos'; document.getElementById(id)?.remove();
  ...
  // initFilters robusto: mapear inputs con data-col/name o th.dataset; ignorar checkbox; debounce + Enter
  function initFilters() {
    const table = document.getElementById(DOM.table);
    if (!table) { console.error('[monitoreos_generales] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[monitoreos_generales] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[monitoreos_generales] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[monitoreos_generales] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      // ignorar checkboxes (columna selección)
      if (inp.type && inp.type.toLowerCase() === 'checkbox') return;
      // determinar nombre columna - priorizar atributo data-col
      let col = '';
      if (inp.dataset && inp.dataset.col) {
        ...
        if (th) {
          if (th.dataset && (th.dataset.col || th.dataset.field)) {
            ...
            // intentar coincidir texto header -> COLUMNAS
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[monitoreos_generales] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
      inp.dataset._col = col;
      // inicializar filtro con valor existente
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[monitoreos_generales] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      // Inmediato al Enter
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[monitoreos_generales] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      // Guardar manejadores en input para remover luego
      inp._filterHandlers = {
        input: handlerDeb,
        change: handlerDeb,
        keydown: handlerKey
      ...
      console.log('[monitoreos_generales] filtro configurado para columna:', col);
    });
    console.log('[monitoreos_generales] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  async function load() {
    try {
      ...
      render();
      // tras render, sincronizar inputs thead con filtros (UI refleja estado)
      const table = document.getElementById(DOM.table);
      if (table) {
        const thead = table.querySelector('thead');
        if (thead) {
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      ...
      console.log('[monitoreos_generales] filtros limpiados, recargando datos...');
      load();
    });
    ...
    // init filters y cargar datos
    initFilters();
    load();
  }
  // Verificar si DOM cargado, sino esperar DOMContentLoaded
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init);
  } else {
    // DOM cargado, llamar init inmediatamente
    init();
  }
})();