 ...
 * aud_perdidas.js
 * 
 * Módulo para: Auditoría de pérdidas
 * 
 * Funcionalidades principales:
 * - Listado paginado de registros de auditoría de pérdidas con filtros dinámicos
 ...
 * - Control y análisis de pérdidas en procesos agrícolas
...
(function(){
  ...
  const DOM={
    tbody:'tbody-aud-perdidas',
    table:'tabla-aud-perdidas',
    pagination:'pagination-aud-perdidas',
    exportBtn:'exportBtnAudPerdidas',
    clearBtn:'clearFiltersBtnAudPerdidas',
    limitSelect:'limitSelectAudPerdidas',
    selectAll:'selectAllAudPerdidas',
    form:'form-editar',
    modal:'modal-editar'
  };
  const COLUMNAS=[
    'aud_perdidas_id','hora','responsable','tipo_auditoria','fecha_actividad','plantacion','siembra','finca','lote','parcela','linea','palma','fruto_suelto_perdido','racimos_perdidos','supervision','error_registro'
  ];
  ...
  function exportar(t){
    if(t==='todo'){ const qs = new URLSearchParams({action:ACTIONS.listFallback[0], page:1, pageSize:100000}); fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'aud_perdidas_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'aud_perdidas_'+t);
  }

  function showExportMenu(){
    const btn=document.getElementById(DOM.exportBtn); if(!btn) return;
    const id='exportMenuAudPerdidas'; document.getElementById(id)?.remove();
  ...
  async function load(){
    try{ const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); }catch(e){ console.warn('[aud_perdidas] error cargar', e); }
  }
