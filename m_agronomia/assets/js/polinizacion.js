 ...
 * polinizacion.js
 * 
 * Módulo para: Registro de polinización
 * 
 * Funcionalidades principales:
 * - Listado paginado de registros de polinización con filtros dinámicos
 ...
 * - Control de actividades de polinización en cultivos
...
(function () {
  ...
  const DOM = {
    ...
    table: 'tabla-polinizacion',
    pagination: 'pagination-polinizacion',
    exportBtn: 'exportBtnPolinizacion',
    clearBtn: 'clearFiltersBtnPolinizacion',
    limitSelect: 'limitSelectPolinizacion',
    selectAll: 'selectAllPolinizacion',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'polinizacion_id', 'fecha', 'colaborador', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'labor', 'linea_entrada',
    'linea_salida', 'hora_entrada', 'hora_salida', 'polen_g', 'talco_g', 'poliniza_6_g', 'ana_98_g', 'acadian_cc', 'cosmoagua_g',
    'agua_cc', 'cosmoflux', 'stimplex_cc', 'oleorganic_cc', 'cosmo_oil_cc', 'etanol_cc', 'flores_dobles', 'flores_buenas', 'flores_ayudadas',
    'aplicacion_2', 'aplicacion_3', 'observaciones', 'supervision', 'error_registro'
  ];
  ...
  function render() {
    ...
    data.forEach((row, i) => {
      ...
      const fecha = row[DATE_COL] || '', inactivo = ((row.observaciones || '') + '|' + (row.error_registro || '')).toLowerCase().includes('inactivo');
      let edit = '', lock = '';
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[polinizacion] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[polinizacion] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[polinizacion] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[polinizacion] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[polinizacion] activar ok', { id, result: j }); load(); }
      else {
  ...
  function openModal(id, readonly) {
    ...
    // Excluir error_registro de inputs del modal (mismo comportamiento que cosecha_fruta)
