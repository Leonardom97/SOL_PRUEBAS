(function(){
  ...
  // ---------------------------------------------------------------
    ...
    const COLUMNAS=[
    'ct_cal_sanidad_id','fecha','hora','responsable','labor','colaborador','plantacion','finca','siembra','lote','parcela','linea','palma','tipo_labor','estado','etapa','instar','verificacion','observaciones','error_registro','supervision'
  ];
  ...
  async function fetchData(){
    let last=''; for(const act of ACTIONS.listFallback){
      try{
        ...
        console.debug('[ct_cal_sanidad] request ->', `${API}?${qs.toString()}`);
  ...
  function initFilters(){
    ...
    if(!table) { console.warn('[ct_cal_sanidad] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if(!thead) { console.warn('[ct_cal_sanidad] thead no encontrado'); return; }
    ...
    inputs.forEach(inp=>{
      ...
      const handlerDeb = debounce(function(evt){
        ...
        filters[col] = val;
        page = 1;
        console.debug('[ct_cal_sanidad] aplicar filtro (debounce):', col, val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e){
        if(e.key === 'Enter'){
          ...
          filters[col] = val;
          page = 1;
          console.debug('[ct_cal_sanidad] aplicar filtro (Enter):', col, val);
          load();
        }
      }
    ...
    console.debug('[ct_cal_sanidad] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
