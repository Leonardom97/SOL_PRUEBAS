...
(function () {
  ...
  const DOM = {
    tbody: 'tbody-labores-diarias',
    table: 'tabla-labores-diarias',
    pagination: 'pagination-labores-diarias',
    exportBtn: 'exportBtnLaboresDiarias',
    clearBtn: 'clearFiltersBtnLaboresDiarias',
    limitSelect: 'limitSelectLaboresDiarias',
    selectAll: 'selectAllLaboresDiarias',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'labores_diarias_id', 'fecha_actividad', 'hora', 'responsable', 'colaborador', 'tipo_labor', 'labor', 'contratista', 'plantacion', 'finca',
    'siembra', 'lote', 'parcela', 'linea', 'palma', 'linea_entrada', 'linea_salida', 'unidad', 'unidad_medida', 'cant_unidad', 'herramienta', 'maquina',
    'horometro_inicial', 'horometro_final', 'hora_inicio', 'hora_final', 'cant_colaborador', 'labor_especifica', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/labores_diarias_api.php';
  const ID_KEY = 'labores_diarias_id';
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[' + ID_KEY + '] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  function exportar(t) {
    if (t === 'todo') { const qs = new URLSearchParams({ action: ACTIONS.listFallback[0], page: 1, pageSize: 100000 }); fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'labores_diarias_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'labores_diarias_' + t);
  }
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuLaboresDiarias'; document.getElementById(id)?.remove();
  ...
  async function load() {
    try { const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); } catch (e) { console.warn('[labores_diarias] error cargar', e); }
  }
  function init() {
