(function(){
  ...
  const COLUMNAS=[
    ...
    'lote','parcela','linea','palma','hallazgo','observacion','error_registro','supervision'
  ];
  ...
  async function fetchData(){
    ...
    for(const act of ACTIONS.listFallback){
      try{
        ...
        let j;
        try{ j = txt ? JSON.parse(txt) : {}; } catch { last=`${act}: JSON`; continue; }
  ...
  async function inactivar(id){
    ...
    try{
      const r=await fetch(`${API}?action=${ACTIONS.inactivate}`,{
        method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({[ID_KEY]:id})
      });
  ...
  function openModal(id,readonly){
    ...
    if(footer){
      ...
      if(row.supervision==='aprobado'||row.check==1){
        ...
        btn.type='button'; btn.className='btn btn-link icon-repeat-supervision';
        btn.title='Revertir aprobación';
        btn.innerHTML='<i class="fa-solid fa-repeat" style="font-size:1.6em;color:#198754;"></i>';
  ...
  async function save(e){
    ...
    try{
      const r=await fetch(`${API}?action=${ACTIONS.save}`,{
  ...
  function exportar(tipo){
    if(tipo==='todo'){
      ...
      fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'reporte_lote_monitoreo_todo'));
      return;
    }
    let rows=data;
    ...
    buildXLSX(rows,'reporte_lote_monitoreo_'+tipo);
  }

  function showExportMenu(){
    const btn=document.getElementById(DOM.exportBtn); if(!btn) return;
    const id='exportMenuRepLote'; document.getElementById(id)?.remove();
  ...
  async function load(){
    ...
    catch(e){ alert(e.message||'Error cargando reporte lote monitoreo'); }
  }
  ...
  function init(){
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click',()=>{
      filters={}; page=1;
      document.querySelectorAll(`#${DOM.table} thead input[data-col]`).forEach(i=>i.value='');
      load();
    });
    document.getElementById(DOM.limitSelect)?.addEventListener('change',e=>{
      pageSize=parseInt(e.target.value,10)||25; page=1; load();
    });
    ...
    document.querySelectorAll(`#${DOM.table} thead .icon-sort`).forEach(icon=>{
      icon.addEventListener('click',()=>{
        if(sortCol===icon.dataset.col) sortAsc=!sortAsc;
        else { sortCol=icon.dataset.col; sortAsc=true; }
        page=1; load();
      });
    });
