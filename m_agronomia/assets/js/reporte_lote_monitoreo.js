(function(){
  ...
  function initFilters(){
    ...
    if(!table) { console.warn('[reporte_lote_monitoreo] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if(!thead) { console.warn('[reporte_lote_monitoreo] thead no encontrado'); return; }
    ...
    inputs.forEach(inp=>{
      ...
      const handlerDeb = debounce(function(evt){
        ...
        filters[col] = val;
        page = 1;
        console.debug('[reporte_lote_monitoreo] aplicar filtro (debounce):', col, val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e){
        if(e.key === 'Enter'){
          ...
          filters[col] = val;
          page = 1;
          console.debug('[reporte_lote_monitoreo] aplicar filtro (Enter):', col, val);
          load();
        }
      }
    ...
    console.debug('[reporte_lote_monitoreo] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
