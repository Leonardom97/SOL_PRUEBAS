...
(function () {
  ...
  const DOM = {
    ...
    pagination: 'pagination-reporte-lote-monitoreo',
    exportBtn: 'exportBtnReporteLote',
    clearBtn: 'clearFiltersBtnReporteLote',
    limitSelect: 'limitSelectReporteLote',
    selectAll: 'selectAllReporteLote',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'reporte_lote_monitoreo_id', 'fecha', 'hora', 'colaborador', 'labor', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'linea',
    'palma', 'hallazgo', 'observacion', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/reporte_lote_monitoreo_api.php';
  const ID_KEY = 'reporte_lote_monitoreo_id';
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[reporte_lote_monitoreo] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch reporte_lote_monitoreo' + ' => ' + last);
  }
  function render() {
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[reporte_lote_monitoreo] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[reporte_lote_monitoreo] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[reporte_lote_monitoreo] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[reporte_lote_monitoreo] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[reporte_lote_monitoreo] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[reporte_lote_monitoreo] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[reporte_lote_monitoreo] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[reporte_lote_monitoreo] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[reporte_lote_monitoreo] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function initFilters() {
    ...
    if (!table) { console.error('[reporte_lote_monitoreo] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[reporte_lote_monitoreo] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[reporte_lote_monitoreo] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[reporte_lote_monitoreo] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[reporte_lote_monitoreo] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[reporte_lote_monitoreo] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[reporte_lote_monitoreo] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      ...
      console.log('[reporte_lote_monitoreo] filtro configurado para columna:', col);
    });
    console.log('[reporte_lote_monitoreo] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  async function load() {
    try {
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      ...
      console.log('[reporte_lote_monitoreo] filtros limpiados, recargando datos...');
      load();
    });
