...
(function () {
  ...
  const DOM = {
    tbody: 'tbody-siembra-nueva',
    table: 'tabla-siembra-nueva',
    pagination: 'pagination-siembra-nueva',
    exportBtn: 'exportBtnSiembraNueva',
    clearBtn: 'clearFiltersBtnSiembraNueva',
    limitSelect: 'limitSelectSiembraNueva',
    selectAll: 'selectAllSiembraNueva',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'siembra_nueva_id', 'fecha', 'hora', 'fecha_actividad', 'responsable', 'labor', 'plantacion', 'finca', 'siembra', 'lote', 'parcela',
    'linea_entrada', 'linea_salida', 'hora_entrada', 'hora_salida', 'contratista', 'codigo', 'colaborador', 'labor_especifica', 'tipo_labor',
    'cantidad_unidad', 'cant_colaborador_siembra', 'operador', 'maquina', 'observacion', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/siembra_nueva_api.php';
  const ID_KEY = 'siembra_nueva_id';
  ...
  function exportar(t) {
    if (t === 'todo') { const qs = new URLSearchParams({ action: ACTIONS.listFallback[0], page: 1, pageSize: 100000 }); fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'siembra_nueva_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'siembra_nueva_' + t);
  }
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuSiembraNueva'; document.getElementById(id)?.remove();
  ...
  async function load() {
    try { const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); } catch (e) { console.warn('[siembra_nueva] error cargar', e); }
  }
  function init() {
