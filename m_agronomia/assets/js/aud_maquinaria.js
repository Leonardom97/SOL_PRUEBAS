(function(){
  ...
  const DOM = {
    tbody: 'tbody-aud-maquinaria',
    table: 'tabla-aud-maquinaria',
    pagination: 'pagination-aud-maquinaria',
    exportBtn: 'exportBtnAudMaquinaria',
    clearBtn: 'clearFiltersBtnAudMaquinaria',
    limitSelect: 'limitSelectAudMaquinaria',
    selectAll: 'selectAllAudMaquinaria',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  ...
  const COLUMNAS = [
    ...
    'tipo_auditoria','error_registro','supervision'
  ];

  const API = 'assets/php/aud_maquinaria_api.php';
  const ID_KEY = 'aud_maquinaria_id';
  ...
  async function fetchData(){
    ...
    for(const act of ACTIONS.listFallback){
      try{
        ...
        console.debug('[aud_maquinaria] request ->', `${API}?${qs.toString()}`);
  ...
  async function inactivar(id){
    ...
    try{
      ...
      if(j.success){ load(); }
        ...
        if(norm==='exception' || norm==='id_required'){ console.warn('[aud_maquinaria] error suprimido:', err); }
        else alert(err);
      }
    } catch(err){
      const msg = err?.message || 'Error inactivar';
      const norm = (msg===null||msg===undefined)?'':String(msg).trim().toLowerCase();
      if(norm==='exception' || norm==='id_required'){ console.warn('[aud_maquinaria] error suprimido:', msg); }
      else alert(msg);
    }
  }
  ...
  async function revertir(id){
    ...
    try{
      ...
      if(!j.success){
        ...
        const norm = (err===null||err===undefined)?'':String(err).trim().toLowerCase();
        if(norm==='exception' || norm==='id_required'){ console.warn('[aud_maquinaria] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg===null||msg===undefined)?'':String(msg).trim().toLowerCase();
      if(norm==='exception' || norm==='id_required'){ console.warn('[aud_maquinaria] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  ...
  async function save(e){
    ...
    try{
      ...
      if(j.success){ alert('Guardado'); load(); setTimeout(()=>bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(),150); }
        ...
        const norm = (err===null||err===undefined)?'':String(err).trim().toLowerCase();
        if(norm==='exception' || norm==='id_required'){ console.warn('[aud_maquinaria] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch(err){
      const msg = err?.message || 'Error guardando';
      const norm = (msg===null||msg===undefined)?'':String(msg).trim().toLowerCase();
      if(norm==='exception' || norm==='id_required'){ console.warn('[aud_maquinaria] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function exportar(t){
    if(t==='todo'){
      ...
      fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'aud_maquinaria_todo')); return;
    }
    let rows = data;
    if(t==='seleccion'){
      rows = []; document.querySelectorAll(`#${DOM.tbody} .row-check`).forEach(ch=>{ if(ch.checked) rows.push(data[+ch.dataset.index]); });
    }
    buildXLSX(rows, 'aud_maquinaria_'+t);
  }

  function showExportMenu(){
    const btn=document.getElementById(DOM.exportBtn); if(!btn) return;
    const id='exportMenuAudMaquinaria'; document.getElementById(id)?.remove();
  ...
  function initFilters(){
    ...
    if(!inputs.length) { console.warn('[aud_maquinaria] no se encontraron inputs en thead'); return; }
    ...
    inputs.forEach(inp=>{
      ...
      if(!col) { console.debug('[aud_maquinaria] input ignored (no col found):', inp); return; }
    ...
    console.debug('[aud_maquinaria] filtros inicializados.');
  }
  ...
  async function load(){
    try{
      ...
      console.warn('[aud_maquinaria] error cargar', e);
    }
  }
