(function(){
    ...
    if(__orig_alert){
      ...
      window.alert = function(message){
        try {
          ...
          if (IGNORED.has(norm)) { console.warn('[suppress_alerts] alert suprimido:', s); return; }
        } catch(e) { console.error('[suppress_alerts] error al evaluar alert:', e); }
  ...
  const COLUMNAS=[
    'ct_polinizacion_flores_id','fecha','hora','responsable','colaborador','auditoria','plantacion','finca','siembra','lote','parcela','linea_entrada','linea_salida','calificacion','verificacion','validacion','flores_dobles','flores_buenas','flores_ayudadas','flores_anteriores_sin_polinizar','flores_nuevas_sin_polinizar','aplicacion_2','aplicacion_3','labor','supervision','error_registro'
  ];
  ...
  function exportar(t){
    if(t==='todo'){ const qs = new URLSearchParams({action:ACTIONS.listFallback[0], page:1, pageSize:100000}); fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'ct_polinizacion_flores_todo')); return; }
    let rows = data;
  ...
  async function load(){
    try{ const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); }catch(e){ console.warn('[ct_polinizacion_flores] error cargar', e); }
  }
