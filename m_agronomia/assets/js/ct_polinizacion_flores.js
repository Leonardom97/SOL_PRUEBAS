...
(function () {
  ...
  // ---------------------------------------------------------------
  const DOM = {
    tbody: 'tbody-ct-polinizacion-flores',
    table: 'tabla-ct-polinizacion-flores',
    pagination: 'pagination-ct-polinizacion-flores',
    exportBtn: 'exportBtnCtPolinizacionFlores',
    clearBtn: 'clearFiltersBtnCtPolinizacionFlores',
    limitSelect: 'limitSelectCtPolinizacionFlores',
    selectAll: 'selectAllCtPolinizacionFlores',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'ct_polinizacion_flores_id', 'fecha', 'hora', 'responsable', 'colaborador', 'auditoria', 'plantacion', 'finca', 'siembra', 'lote', 'parcela',
    'linea_entrada', 'linea_salida', 'calificacion', 'verificacion', 'validacion', 'flores_dobles', 'flores_buenas', 'flores_ayudadas',
    'flores_anteriores_sin_polinizar', 'flores_nuevas_sin_polinizar', 'aplicacion_2', 'aplicacion_3', 'labor', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/ct_polinizacion_flores_api.php';
  const ID_KEY = 'ct_polinizacion_flores_id';
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.debug('[ct_polinizacion_flores] request ->', `${API}?${qs.toString()}`);
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[ct_polinizacion_flores] toggle error_registro ok', { id, action, result: j });
        await load();
      } else {
        const err = j.error || 'Fallo acción';
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_polinizacion_flores] error suprimido:', err); }
        else { alert(err); }
        switchElement.checked = !!prevWasActive;
      }
    } catch (err) {
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_polinizacion_flores] error suprimido:', msg); }
      else { alert(msg); }
      switchElement.checked = !!prevWasActive;
    }
  }
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[ct_polinizacion_flores] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[ct_polinizacion_flores] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_polinizacion_flores] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_polinizacion_flores] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_polinizacion_flores] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_polinizacion_flores] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function exportar(t) {
    if (t === 'todo') {
      ...
      fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'ct_polinizacion_flores_todo')); return;
    }
    let rows = data;
    if (t === 'seleccion') {
      rows = []; document.querySelectorAll(`#${DOM.tbody} .row-check`).forEach(ch => { if (ch.checked) rows.push(data[+ch.dataset.index]); });
    }
    buildXLSX(rows, 'ct_polinizacion_flores_' + t);
  }
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuCtPolinizacionFlores'; document.getElementById(id)?.remove();
  ...
  function initFilters() {
    ...
    if (!table) { console.warn('[ct_polinizacion_flores] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.warn('[ct_polinizacion_flores] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    if (!inputs.length) { console.warn('[ct_polinizacion_flores] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) {
        console.debug('[ct_polinizacion_flores] input ignored (no col found):', inp);
        return;
      }
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.debug('[ct_polinizacion_flores] aplicar filtro (debounce):', col, val);
        load();
      }, FILTER_DEBOUNCE_MS);
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.debug('[ct_polinizacion_flores] aplicar filtro (Enter):', col, val);
          load();
        }
      }
    ...
    console.debug('[ct_polinizacion_flores] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
  async function load() {
    try {
      ...
      console.warn('[ct_polinizacion_flores] error cargar', e);
    }
  }
  function init() {
