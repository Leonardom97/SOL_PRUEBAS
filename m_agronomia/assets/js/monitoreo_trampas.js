(function(){
  ...
  const COLUMNAS=[
    'monitoreo_trampas_id','fecha','hora','colaborador','labor','ubicacion','plantacion','finca','siembra',
    'lote','parcela','tipo_trampa','linea','palma','plaga','hembra','macho','lado_a','lado_b','numero_trampa',
    'estado_lona','estado_trampa','estado_ventana','estado_cania','estado_melaza','estado_feromona',
    'estado_tapa','estado_envase','observacion','error_registro','supervision'
  ];
  ...
  function render(){
    ...
    data.forEach((row,i)=>{
      const est=estado(row); const tr=document.createElement('tr');
  ...
  async function inactivar(id){
    ...
    try{
      ...
      const j=await r.json(); if(j.success){alert('Inactivado'); load();} else alert(j.error||'Fallo inactivar');
  ...
  async function load(){
    ...
    catch(e){ alert(e.message||'Error cargando monitoreo_trampas'); }
  }
  function init(){
