...
(function () {
  ...
  // ---------------------------------------------------------------
  const DOM = {
    tbody: 'tbody-monitoreo-trampas',
    table: 'tabla-monitoreo-trampas',
    pagination: 'pagination-monitoreo-trampas',
    exportBtn: 'exportBtnTrampas',
    clearBtn: 'clearFiltersBtnTrampas',
    limitSelect: 'limitSelectTrampas',
    selectAll: 'selectAllTrampas',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'monitoreo_trampas_id', 'fecha', 'hora', 'colaborador', 'labor', 'ubicacion', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'tipo_trampa',
    'linea', 'palma', 'plaga', 'hembra', 'macho', 'lado_a', 'lado_b', 'numero_trampa', 'estado_lona', 'estado_trampa', 'estado_ventana', 'estado_cania',
    'estado_melaza', 'estado_feromona', 'estado_tapa', 'estado_envase', 'observacion', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/monitoreo_trampas_api.php';
  const ID_KEY = 'monitoreo_trampas_id';
  ...
  // Debounce para inputs de filtro
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        // agregar filtros normalizados no vacíos
        ...
        // debug: mostrar url de petición en consola
        console.log('[monitoreo_trampas] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch monitoreo_trampas' + ' => ' + last);
  }
  function render() {
  ...
  function bindRowEvents() {
    ...
    t.querySelectorAll('.error-reg-switch').forEach(input => {
      input.onchange = (e) => {
        ...
        const prevState = !input.checked; // estado previo (antes del cambio)
        toggleErrorRegistro(id, prevState, input);
      };
    });
  }
  // Alternar estado de error_registro: si switch marcado => activar, sino => inactivar
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[monitoreo_trampas] toggle error_registro ok', { id, action, result: j });
        // recargar datos para reflejar badge y otros cambios de UI
        await load();
      } else {
        const err = j.error || 'Fallo acción';
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreo_trampas] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreo_trampas] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[monitoreo_trampas] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    // mantenido por compatibilidad pero no vinculado directamente ahora
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[monitoreo_trampas] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreo_trampas] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreo_trampas] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    // Omitir si campo ID no está presente o vacío (form de otra entidad)
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreo_trampas] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[monitoreo_trampas] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function exportar(t) {
    if (t === 'todo') {
      ...
      fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'monitoreo_trampas_todo')); return;
    }
    let rows = data;
    if (t === 'seleccion') {
      rows = []; document.querySelectorAll(`#${DOM.tbody} .row-check`).forEach(ch => { if (ch.checked) rows.push(data[+ch.dataset.index]); });
    }
    buildXLSX(rows, 'monitoreo_trampas_' + t);
  }
  // Estilo de menú de exportación restaurado
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuTrampas'; document.getElementById(id)?.remove();
  ...
  // initFilters robusto: mapear inputs usando data-col/name o th.dataset; ignorar checkboxes; agregar debounce + Enter
  function initFilters() {
    const table = document.getElementById(DOM.table);
    if (!table) { console.error('[monitoreo_trampas] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[monitoreo_trampas] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[monitoreo_trampas] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[monitoreo_trampas] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      // ignorar checkboxes (columna de selección)
      if (inp.type && inp.type.toLowerCase() === 'checkbox') return;
      // determinar nombre de columna - priorizar atributo data-col
      let col = '';
      if (inp.dataset && inp.dataset.col) {
        ...
        if (th) {
          if (th.dataset && (th.dataset.col || th.dataset.field)) {
            ...
            // intentar coincidir texto de cabecera -> COLUMNAS
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[monitoreo_trampas] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[monitoreo_trampas] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      // Inmediato al presionar Enter
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[monitoreo_trampas] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      // Guardar manejadores en el elemento input para posterior eliminación
      inp._filterHandlers = {
        input: handlerDeb,
        change: handlerDeb,
        keydown: handlerKey
      ...
      console.log('[monitoreo_trampas] filtro configurado para columna:', col);
    });
    console.log('[monitoreo_trampas] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  async function load() {
    try {
      ...
      render();
      // después de renderizar, sincronizar inputs del thead con filtros (para que UI refleje estado actual)
      const table = document.getElementById(DOM.table);
      if (table) {
        const thead = table.querySelector('thead');
        if (thead) {
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      ...
      console.log('[monitoreo_trampas] filtros limpiados, recargando datos...');
      load();
    });
  ...
  // Verificar si DOM ya está cargado, sino esperar DOMContentLoaded
