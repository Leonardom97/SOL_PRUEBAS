(function(){
  ...
  const COLUMNAS=[
    ...
    'labor_especifica','tipo_corte','equipo','cod_colaborador_contratista','n_grupo_dia',
    'hora_entrada','hora_salida','linea_entrada','linea_salida','total_personas','unidad','cantidad',
    'peso_promedio_lonas','total_persona_dia','colaborador','nuevo_operador','error_registro','supervision'
  ];
  ...
  function exportar(tipo){
    if(tipo==='todo'){
      ...
      fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'cosecha_fruta_todo'));
      return;
    }
    let rows=data;
    if(tipo==='seleccion'){
    ...
    buildXLSX(rows,'cosecha_fruta_'+tipo);
  }
