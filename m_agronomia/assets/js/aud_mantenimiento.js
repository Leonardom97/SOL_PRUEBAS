...
(function(){
  ...
  const DOM={
    tbody:'tbody-aud-mantenimiento',
    table:'tabla-aud-mantenimiento',
    pagination:'pagination-aud-mantenimiento',
    exportBtn:'exportBtnAudMantenimiento',
    clearBtn:'clearFiltersBtnAudMantenimiento',
    limitSelect:'limitSelectAudMantenimiento',
    selectAll:'selectAllAudMantenimiento',
    form:'form-editar',
    modal:'modal-editar'
  };
  const COLUMNAS=[
    'aud_mantenimiento_id','hora','responsable','labor_especifica','fecha_actividad','plantacion','siembra','finca','lote','parcela','tipo_auditoria','linea_entrada','linea_salida','calificacion_m','linea_m','palma_m','observaciones','supervision','error_registro'
  ];
  const API='assets/php/aud_mantenimiento_api.php';
  const ID_KEY='aud_mantenimiento_id';
  ...
  function exportar(t){
    if(t==='todo'){ const qs = new URLSearchParams({action:ACTIONS.listFallback[0], page:1, pageSize:100000}); fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'aud_mantenimiento_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'aud_mantenimiento_'+t);
  }

  function showExportMenu(){
    const btn=document.getElementById(DOM.exportBtn); if(!btn) return;
    const id='exportMenuAudMantenimiento'; document.getElementById(id)?.remove();
  ...
  async function load(){
    try{ const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); }catch(e){ console.warn('[aud_mantenimiento] error cargar', e); }
  }
