(function(){
  ...
  const COLUMNAS=[
    ...
    'unidad_aplicacion','contratista_colaborador','n_colaboradores','colaboradores','tipo_labor',
    'contratista_maquinaria','n_operadores','tipo_maquina','nombre_operadores','bultos_aplicados',
    'n_traslado','kg_aplicados','error_registro','supervision'
  ];
  ...
  async function fetchData(){
    let last=''; for(const act of ACTIONS.listFallback){
      try{
        ...
        console.debug('[fertilizacion_organica] request ->', `${API}?${qs.toString()}`);
  ...
  function showExportMenu(){
    ...
    const id='exportMenuFertOrg'; document.getElementById(id)?.remove();
  ...
  function initFilters(){
    const table = document.getElementById(DOM.table);
    if(!table) { console.warn('[fertilizacion_organica] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if(!thead) { console.warn('[fertilizacion_organica] thead no encontrado'); return; }
    ...
    inputs.forEach(inp=>{
      ...
      const handlerDeb = debounce(function(evt){
        ...
        filters[col] = val;
        page = 1;
        console.debug('[fertilizacion_organica] aplicar filtro (debounce):', col, val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e){
        if(e.key === 'Enter'){
          ...
          filters[col] = val;
          page = 1;
          console.debug('[fertilizacion_organica] aplicar filtro (Enter):', col, val);
          load();
        }
      }
    ...
    console.debug('[fertilizacion_organica] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
