...
(function () {
  ...
  // ---------------------------------------------------------------
  const DOM = {
    tbody: 'tbody-coberturas',
    table: 'tabla-coberturas',
    pagination: 'pagination-coberturas',
    exportBtn: 'exportBtnCoberturas',
    clearBtn: 'clearFiltersBtnCoberturas',
    limitSelect: 'limitSelectCoberturas',
    selectAll: 'selectAllCoberturas',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'coberturas_id', 'fecha', 'hora', 'fecha_actividad', 'responsable', 'contratista', 'codigo', 'colaborador', 'labor', 'plantacion', 'finca',
    'siembra', 'lote', 'parcela', 'linea_entrada', 'linea_salida', 'unidad', 'cantidad', 'h_inicio', 'h_fin', 'cobertura', 'cant_colaboradores',
    'operador', 'maquina', 'observaciones', 'supervision', 'error_registro'
  ];
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.debug('[coberturas] request ->', `${API}?${qs.toString()}`); // debug: request url
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[coberturas] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[coberturas] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[coberturas] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[coberturas] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[coberturas] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[coberturas] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[coberturas] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[coberturas] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[coberturas] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function exportar(t) {
    if (t === 'todo') {
      ...
      fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'coberturas_todo')); return;
    }
    let rows = data;
    if (t === 'seleccion') {
      rows = []; document.querySelectorAll(`#${DOM.tbody} .row-check`).forEach(ch => { if (ch.checked) rows.push(data[+ch.dataset.index]); });
    }
    buildXLSX(rows, 'coberturas_' + t);
  }
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuCoberturas'; document.getElementById(id)?.remove();
  ...
  function initFilters() {
    ...
    if (!table) { console.warn('[coberturas] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.warn('[coberturas] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    if (!inputs.length) { console.warn('[coberturas] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) {
        console.debug('[coberturas] input ignored (no col found):', inp);
        return;
      }
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.debug('[coberturas] aplicar filtro (debounce):', col, val);
        load();
      }, FILTER_DEBOUNCE_MS);
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.debug('[coberturas] aplicar filtro (Enter):', col, val);
          load();
        }
      }
    ...
    console.debug('[coberturas] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
  async function load() {
    try {
      ...
      console.warn('[coberturas] error cargar', e);
    }
  }
  function init() {
