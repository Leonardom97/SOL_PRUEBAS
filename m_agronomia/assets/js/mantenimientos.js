...
(function () {
  ...
  const COLUMNAS = [
    'mantenimientos_id', 'fecha', 'responsable', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'labor_especifica', 'observacion',
    'contratista', 'codigo', 'colaborador', 'personas', 'hora_entrada', 'hora_salida', 'linea_entrada', 'linea_salida', 'cantidad', 'unidad',
    'maquina', 'tractorista', 'nuevo_operario', 'supervision', 'error_registro'
  ];
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[mantenimientos] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[mantenimientos] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[mantenimientos] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[mantenimientos] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[mantenimientos] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[mantenimientos] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[mantenimientos] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[mantenimientos] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[mantenimientos] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function exportar(t) {
    if (t === 'todo') {
      ...
      fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'mantenimientos_todo')); return;
    }
    let rows = data;
    if (t === 'seleccion') {
  ...
  function showExportMenu() {
    ...
    const id = 'exportMenuMantenimientos'; document.getElementById(id)?.remove();
  ...
  function initFilters() {
    ...
    if (!table) { console.warn('[mantenimientos] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.warn('[mantenimientos] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    if (!inputs.length) { console.warn('[mantenimientos] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
    ...
    console.debug('[mantenimientos] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
  async function load() {
    try {
      ...
      console.warn('[mantenimientos] error cargar', e);
    }
  }
  function init() {
