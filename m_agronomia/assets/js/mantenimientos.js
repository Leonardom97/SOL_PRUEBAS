(function(){
  ...
  const COLUMNAS=[
    'mantenimientos_id','fecha','responsable','plantacion','finca','siembra','lote','parcela','labor_especifica','observacion','contratista','codigo','colaborador','personas','hora_entrada','hora_salida','linea_entrada','linea_salida','cantidad','unidad','maquina','tractorista','nuevo_operario','supervision','error_registro'
  ];
  ...
  function openModal(id,readonly){
    ...
    if(footer){
      ...
      // Show revert button only if approved AND user has permission
      const perms = window.AGRONOMIA_PERMISSIONS || {};
  ...
  async function revertir(id){
    if(!confirm('¿Revertir aprobación? El registro volverá a estado pendiente.')) return;
    try{
      ...
      const j = await r.json();
      if(!j.success){
        const err = j.error || j.message || 'No revertido';
        const norm = (err===null||err===undefined)?'':String(err).trim().toLowerCase();
        if(norm==='exception' || norm==='id_required'){ console.warn('[mantenimientos] error suprimido:', err); return; }
        alert(err); return;
      }
      alert('Registro revertido a pendiente');
      await load(); 
      setTimeout(()=>bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(),150);
    }catch(err){
