...
(function () {
  ...
  const DOM = {
    tbody: 'tbody-salida-vivero',
    table: 'tabla-salida-vivero',
    pagination: 'pagination-salida-vivero',
    exportBtn: 'exportBtnSalidaVivero',
    clearBtn: 'clearFiltersBtnSalidaVivero',
    limitSelect: 'limitSelectSalidaVivero',
    selectAll: 'selectAllSalidaVivero',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'salida_vivero_id', 'fecha', 'hora', 'fecha_actividad', 'colaborador', 'labor', 'plantacion', 'finca', 'siembra', 'lote', 'parcela',
    'variedad', 'cantidad', 'recibe', 'transporte', 'numero', 'observacion_vivero', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/salida_vivero_api.php';
  const ID_KEY = 'salida_vivero_id';
  ...
  function exportar(t) {
    if (t === 'todo') { const qs = new URLSearchParams({ action: ACTIONS.listFallback[0], page: 1, pageSize: 100000 }); fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'salida_vivero_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'salida_vivero_' + t);
  }
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuSalidaVivero'; document.getElementById(id)?.remove();
  ...
  async function load() {
    try { const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); } catch (e) { console.warn('[salida_vivero] error cargar', e); }
  }
  function init() {
