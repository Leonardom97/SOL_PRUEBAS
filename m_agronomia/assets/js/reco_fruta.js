 ...
 * reco_fruta.js
 * 
 * Módulo para: Registro de reco fruta
 * 
 * Funcionalidades principales:
 * - Listado paginado de registros de reco fruta con filtros dinámicos
 ...
 * - Seguimiento de racimos, kilos y otras métricas de reco fruta
 */
...
(function () {
  ...
  const API = 'assets/php/reco_fruta_api.php';
  const ID_KEY = 'reco_fruta_id';
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[reco_fruta] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch reco_fruta' + ' => ' + last);
  }
  ...
  function bindRowEvents() {
    ...
    // re-bind selection checkboxes if needed
    t.querySelectorAll('.row-check').forEach(ch => {
      // keep as-is or add listeners if needed
    });
  }
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[reco_fruta] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[reco_fruta] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[reco_fruta] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    // mantenido por compatibilidad hacia atrás pero no vinculado directamente ahora
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[reco_fruta] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[reco_fruta] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[reco_fruta] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[reco_fruta] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  ...
  async function save(e) {
    ...
    // Omitir si el campo ID para esta entidad no está presente o vacío (el formulario es para otra entidad)
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[reco_fruta] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[reco_fruta] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  // initFilters robusto: mapear inputs usando data-col/name o th.dataset; ignorar checkboxes; añadir debounce + Enter
  function initFilters() {
    const table = document.getElementById(DOM.table);
    if (!table) {
      console.error('[reco_fruta] tabla no encontrada:', DOM.table);
      return;
    }
    const thead = table.querySelector('thead');
    if (!thead) {
      console.error('[reco_fruta] thead no encontrado');
      return;
    }

    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    if (!inputs.length) {
      console.warn('[reco_fruta] no se encontraron inputs en thead');
      return;
    }

    console.log('[reco_fruta] inicializando filtros para', inputs.length, 'inputs');
    ...
    inputs.forEach(inp => {
      ...
      if (inp.dataset && inp.dataset.col) {
        ...
        if (th) {
          if (th.dataset && (th.dataset.col || th.dataset.field)) {
            ...
            // intentar coincidir texto de encabezado -> COLUMNAS
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[reco_fruta] input ignored (no col found):', inp);
        return;
      }
      ...
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[reco_fruta] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }

      // Almacenar manejadores en el elemento input para su eliminación posterior
      inp._filterHandlers = {
        input: handlerDeb,
        change: handlerDeb,
        keydown: handlerKey
      ...
      console.log('[reco_fruta] filtro configurado para columna:', col);
    });

    console.log('[reco_fruta] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  ...
  async function load() {
    try {
      ...
      // después de renderizar, sincronizar inputs de thead con filtros (para que la UI refleje el estado actual del filtro)
      const table = document.getElementById(DOM.table);
      if (table) {
        const thead = table.querySelector('thead');
        if (thead) {
      ...
      console.warn('[reco_fruta] error cargar', e);
    }
  }
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      console.log('[reco_fruta] limpiando todos los filtros');
      filters = {}; page = 1;
      const table = document.getElementById(DOM.table);
      if (table) {
        const thead = table.querySelector('thead');
      ...
      console.log('[reco_fruta] filtros limpiados, recargando datos...');
      load();
    });
  ...
  // Verificar si el DOM ya está cargado, de lo contrario esperar a DOMContentLoaded
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init);
  } else {
    // El DOM ya está cargado, llamar a init inmediatamente
    init();
  }
