(function(){
  ...
  function initFilters(){
    ...
    if(!table) { console.warn('[plagas] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if(!thead) { console.warn('[plagas] thead no encontrado'); return; }
    ...
    inputs.forEach(inp=>{
      ...
      const handlerDeb = debounce(function(evt){
        ...
        filters[col] = val;
        page = 1;
        console.debug('[plagas] aplicar filtro (debounce):', col, val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e){
        if(e.key === 'Enter'){
          ...
          filters[col] = val;
          page = 1;
          console.debug('[plagas] aplicar filtro (Enter):', col, val);
          load();
        }
      }
    ...
    console.debug('[plagas] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
