 ...
 * plagas.js
 * 
 * Módulo para: Gestión de plagas
 * 
 * Funcionalidades principales:
 * - Listado paginado de registros de plagas con filtros dinámicos
...
(function () {
  ...
  const COLUMNAS = [
    ...
    'orden', 'plaga', 'etapa', 'cantidad', 'instar', 'estado', 'supervision', 'error_registro'
  ];
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[plagas] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch plagas' + ' => ' + last);
  }
  function render() {
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[plagas] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[plagas] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[plagas] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[plagas] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[plagas] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[plagas] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[plagas] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[plagas] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[plagas] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function initFilters() {
    ...
    if (!table) { console.error('[plagas] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[plagas] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[plagas] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[plagas] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[plagas] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[plagas] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      // Inmediato al Enter
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[plagas] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      ...
      console.log('[plagas] filtro configurado para columna:', col);
    });
    console.log('[plagas] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  async function load() {
    try {
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      console.log('[plagas] limpiando todos los filtros');
      filters = {}; page = 1;
      const table = document.getElementById(DOM.table);
      if (table) {
        const thead = table.querySelector('thead');
      ...
      console.log('[plagas] filtros limpiados, recargando datos...');
      load();
    });
