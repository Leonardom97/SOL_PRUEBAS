 ...
 * resiembra.js
 * 
 * Módulo para: Registro de resiembras
 * 
 * Funcionalidades principales:
 * - Listado paginado de registros de resiembra con filtros dinámicos
 ...
 * - Control de resiembras por pérdida de plantas o renovación
...
(function () {
  ...
  const DOM = {
    tbody: 'tbody-resiembra',
    table: 'tabla-resiembra',
    pagination: 'pagination-resiembra',
    exportBtn: 'exportBtnResiembra',
    clearBtn: 'clearFiltersBtnResiembra',
    limitSelect: 'limitSelectResiembra',
    selectAll: 'selectAllResiembra',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'resiembra_id', 'fecha', 'hora', 'fecha_actividad', 'colaborador', 'labor', 'contratista', 'plantacion', 'finca', 'siembra',
    'lote', 'parcela', 'linea', 'palma', 'variedad_res', 'observacion_res', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/resiembra_api.php';
  const ID_KEY = 'resiembra_id';
  ...
  function exportar(t) {
    if (t === 'todo') { const qs = new URLSearchParams({ action: ACTIONS.listFallback[0], page: 1, pageSize: 100000 }); fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'resiembra_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'resiembra_' + t);
  }
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuResiembra'; document.getElementById(id)?.remove();
  ...
  async function load() {
    try { const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); } catch (e) { console.warn('[resiembra] error cargar', e); }
  }
  function init() {
