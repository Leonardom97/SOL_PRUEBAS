...
(function () {
  ...
  // ---------------------------------------------------------------
  const DOM = {
    tbody: 'tbody-erradicaciones',
    table: 'tabla-erradicaciones',
    pagination: 'pagination-erradicaciones',
    exportBtn: 'exportBtnErradicaciones',
    clearBtn: 'clearFiltersBtnErradicaciones',
    limitSelect: 'limitSelectErradicaciones',
    selectAll: 'selectAllErradicaciones',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
  ...
  async function fetchData() {
    ...
    for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.debug('[erradicaciones] request ->', `${API}?${qs.toString()}`);
  ...
  function render() {
    ...
    data.forEach((row, i) => {
      ...
      else if (isAsistAgronomico) { /* Sin botón de edición para Asist_Agronómico */ }
      else if (corte && fecha) {
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      if (j.success) { load(); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[erradicaciones] error suprimido:', err); }
        else alert(err);
      }
    } catch (err) {
      const msg = err?.message || 'Error inactivar';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[erradicaciones] error suprimido:', msg); }
      else alert(msg);
    }
  }
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[erradicaciones] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[erradicaciones] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[erradicaciones] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[erradicaciones] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function exportar(t) {
    if (t === 'todo') {
      ...
      fetch(`${API}?${qs.toString()}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'erradicaciones_todo')); return;
    }
    let rows = data;
    if (t === 'seleccion') {
      rows = []; document.querySelectorAll(`#${DOM.tbody} .row-check`).forEach(ch => { if (ch.checked) rows.push(data[+ch.dataset.index]); });
    }
    buildXLSX(rows, 'erradicaciones_' + t);
  }
  function showExportMenu() {
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuErradicaciones'; document.getElementById(id)?.remove();
  ...
  // Inicializar filtros mapeando inputs a columnas de forma robusta
  function initFilters() {
    const table = document.getElementById(DOM.table); if (!table) { console.warn('[erradicaciones] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead'); if (!thead) { console.warn('[erradicaciones] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    if (!inputs.length) { console.warn('[erradicaciones] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) { console.debug('[erradicaciones] input ignored (no col found):', inp); return; }
    ...
    console.debug('[erradicaciones] filtros inicializados.');
  }
  async function load() {
    try {
      ...
      render();
      // sincronizar inputs thead con filtros
      ...
      console.warn('[erradicaciones] error cargar', e);
    }
  }
  function init() {
