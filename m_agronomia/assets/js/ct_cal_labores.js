 ...
 * ct_cal_labores.js
 * 
 * Módulo para: Control de calidad de labores
 * 
 * Funcionalidades principales:
 * - Listado paginado de registros de control de calidad de labores con filtros dinámicos
 ...
 * - Evaluación y seguimiento de la calidad de las labores agrícolas
...
(function () {
  ...
  const DOM = {
    ...
    pagination: 'pagination-ct-cal-labores',
    exportBtn: 'exportBtnLabores',
    clearBtn: 'clearFiltersBtnLabores',
    limitSelect: 'limitSelectLabores',
    selectAll: 'selectAllLabores',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'ct_cal_labores_id', 'fecha', 'hora', 'colaborador', 'tipo_auditoria', 'labor_especifica', 'labor', 'plantacion', 'finca',
    'siembra', 'lote', 'parcela', 'linea', 'palma', 'linea_entrada', 'linea_salida', 'tipo_validacion', 'grado_enfermedad', 'numero_galerias',
    'promedio', 'estado', 'producto', 'observación', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/ct_cal_labores_api.php';
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[ct_cal_labores] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch ct_cal_labores' + ' => ' + last);
  }
  function render() {
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[ct_cal_labores] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_labores] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_labores] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[ct_cal_labores] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[ct_cal_labores] activar ok', { id, result: j }); load(); }
      else {
  ...
  function exportar(t) {
    if (t === 'todo') {
      ...
      fetch(`${API}?${qs}`).then(r => r.json()).then(j => buildXLSX((j.datos || j.data || []), 'ct_cal_labores_todo')); return;
    }
    let rows = data;
    if (t === 'seleccion') {
  ...
  function initFilters() {
    ...
    if (!table) { console.error('[ct_cal_labores] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[ct_cal_labores] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[ct_cal_labores] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[ct_cal_labores] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[ct_cal_labores] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[ct_cal_labores] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[ct_cal_labores] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      ...
      console.log('[ct_cal_labores] filtro configurado para columna:', col);
    });
    console.log('[ct_cal_labores] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  async function load() {
    try {
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      ...
      console.log('[ct_cal_labores] filtros limpiados, recargando datos...');
      load();
    });
