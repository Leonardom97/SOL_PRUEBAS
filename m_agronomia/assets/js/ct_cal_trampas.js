...
(function () {
  ...
  const DOM = {
    ...
    pagination: 'pagination-ct-cal-trampas',
    exportBtn: 'exportBtnTrampasCal',
    clearBtn: 'clearFiltersBtnTrampasCal',
    limitSelect: 'limitSelectTrampasCal',
    selectAll: 'selectAllTrampasCal',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'ct_cal_trampas_id', 'fecha', 'hora', 'responsable', 'colaborador', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'trampa', 'plaga',
    'hembra', 'macho', 'lado_a', 'lado_b', 'estado_lona', 'estado_trampa', 'estado_ubicacion', 'estado_ventana', 'estado_cania', 'estado_melaza',
    'estado_feromona', 'calificación', 'observaciones', 'supervision', 'error_registro'
  ];
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[ct_cal_trampas] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch ct_cal_trampas' + ' => ' + last);
  }
  function render() {
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[ct_cal_trampas] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_trampas] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_trampas] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[ct_cal_trampas] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[ct_cal_trampas] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_trampas] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_trampas] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_trampas] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[ct_cal_trampas] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function initFilters() {
    ...
    if (!table) { console.error('[ct_cal_trampas] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[ct_cal_trampas] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[ct_cal_trampas] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[ct_cal_trampas] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[ct_cal_trampas] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[ct_cal_trampas] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[ct_cal_trampas] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      ...
      console.log('[ct_cal_trampas] filtro configurado para columna:', col);
    });
    console.log('[ct_cal_trampas] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  async function load() {
    try {
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      ...
      console.log('[ct_cal_trampas] filtros limpiados, recargando datos...');
      load();
    });
