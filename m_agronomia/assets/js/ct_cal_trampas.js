(function(){
  ...
  async function fetchData(){
    ...
    for (const act of ACTIONS.listFallback) {
      try {
        ...
        last = `${act}: ${e && e.message ? e.message : String(e)}`;
      }
    }
  ...
  async function save(e){
    ...
    // ID fallbacks y envío tanto ID_KEY como id para compatibilidad
    ...
    try { const rol = (document.body.getAttribute('data-role') || '').toLowerCase(); if (!/administrador|aux_agronomico/.test(rol)) obj.supervision = obj.supervision || 'pendiente'; } catch (e) {}

    let resp;
    try {
      resp = await fetch(`${API}?action=${ACTIONS.save}`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(obj), cache: 'no-store' });
    } catch (networkErr) {
      console.error('save network error', networkErr);
      return;
    }

    let j = null;
    try { j = await resp.json(); } catch (parseErr) { try { const txt = await resp.text(); console.warn('save: non-json response', txt); } catch(_) {} console.error('save parse error', parseErr); return; }

    if (j && (j.success === true || j.success === 'true' || j.ok === true)) {
      try { alert('Guardado correctamente'); } catch (_) {}
      try { if (typeof load === 'function') load(); } catch (e) { console.warn('load after save failed', e); }
      try { setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); } catch (_) {}
      return;
    } else {
  ...
  function exportar(tipo){
    ...
    buildXLSX(rows, 'ct_cal_trampas_' + tipo);
  }

  function showExportMenu(){
    const btn = document.getElementById(DOM.exportBtn); if (!btn) return;
    const id = 'exportMenuTrampCal'; document.getElementById(id)?.remove();
    ...
    menu.innerHTML = ['todo','filtrado','seleccion'].map(t => `<button class="exp" data-t="${t}" style="display:block;border:none;background:none;padding:6px 12px;text-align:left;cursor:pointer">${t[0].toUpperCase()+t.slice(1)} (.xlsx)</button>`).join('');
  ...
  async function load(){ try { const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); } catch (e) { alert(e.message || 'Error cargando ct_cal_trampas'); } }
  ...
  function init(){
    ...
    document.querySelectorAll(`#${DOM.table} thead input[data-col]`).forEach(inp => { inp.addEventListener('input', () => { filters[inp.dataset.col] = inp.value; page = 1; load(); }); });
