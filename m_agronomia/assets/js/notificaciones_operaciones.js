/**
 * Modal de pendientes de aprobación.
 * Variante segura: no modifica globals; añade confirmación y envía ambos ids
 * específicamente para cosecha_fruta al rechazar (compatibilidad).
 *
 * Incluir DESPUÉS de init_noti_admin.js y antes de cerrar </body>.
 */
      ...
      if(ent === 'cosecha_fruta' && isReject){
        ...
        .then(r=>{
          ...
          if(r && r.success){
            ...
            // mostrar error para que el usuario sepa (no se suprime)
            const serverMsg = r && (r.message || r.error) ? (r.message || r.error) : JSON.stringify(r);
            window.alert('No se pudo rechazar: ' + serverMsg);
        ...
        .then(r=>{
          ...
          if(r && r.success){
            ...
            if(r && (r.error==='id_required' || r.error==='exception')){
              ...
              // mostrar mensaje para visibilidad
              const msg = r && (r.message || r.error) ? (r.message || r.error) : JSON.stringify(r);
              window.alert('No se pudo completar la acción: '+msg);
            }
          }
        })
        .catch(err=>{
