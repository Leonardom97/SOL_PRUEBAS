(function(){
  ...
  const COLUMNAS=[
    'nivel_freatico_id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela',
    'linea','palma','n_pozo_observacion','superficie_tubo','profundidad_agua','verificacion',
    'validacion','enterrado','nivel_freatico','error_registro','supervision'
  ];
  ...
  async function inactivar(id){
    ...
    try{
      ...
      const j=await r.json(); if(j.success){ console.log('[nivel_freatico] inactivar ok', {id, result:j}); load(); }
      else {
  ...
  async function load(){
    ...
    catch(e){ alert(e.message||'Error cargando nivel_freatico'); }
  }
  function init(){
