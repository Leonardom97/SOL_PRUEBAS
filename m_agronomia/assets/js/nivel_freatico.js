(function(){
  ...
  function initFilters(){
    ...
    if(!table) { console.warn('[nivel_freatico] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if(!thead) { console.warn('[nivel_freatico] thead no encontrado'); return; }
    ...
    inputs.forEach(inp=>{
      ...
      const handlerDeb = debounce(function(evt){
        ...
        filters[col] = val;
        page = 1;
        console.debug('[nivel_freatico] aplicar filtro (debounce):', col, val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e){
        if(e.key === 'Enter'){
          ...
          filters[col] = val;
          page = 1;
          console.debug('[nivel_freatico] aplicar filtro (Enter):', col, val);
          load();
        }
      }
    ...
    console.debug('[nivel_freatico] filtros inicializados. columnas mapeadas:', Object.keys(filters).length ? Object.keys(filters) : 'none');
  }
