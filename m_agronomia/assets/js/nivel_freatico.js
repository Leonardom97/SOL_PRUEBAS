...
(function () {
  ...
  // ---------------------------------------------------------------
  const DOM = {
    tbody: 'tbody-nivel-freatico',
    table: 'tabla-nivel-freatico',
    pagination: 'pagination-nivel-freatico',
    exportBtn: 'exportBtnFreatico',
    clearBtn: 'clearFiltersBtnFreatico',
    limitSelect: 'limitSelectFreatico',
    selectAll: 'selectAllFreatico',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'nivel_freatico_id', 'fecha', 'hora', 'colaborador', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'linea', 'palma',
    'n_pozo_observacion', 'superficie_tubo', 'profundidad_agua', 'verificacion', 'validacion', 'enterrado', 'nivel_freatico',
    'supervision', 'error_registro'
  ];
  const API = 'assets/php/nivel_freatico_api.php';
  const ID_KEY = 'nivel_freatico_id';
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[nivel_freatico] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch nivel_freatico' + ' => ' + last);
  }
  function render() {
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[nivel_freatico] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[nivel_freatico] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[nivel_freatico] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[nivel_freatico] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[nivel_freatico] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function initFilters() {
    ...
    if (!table) { console.error('[nivel_freatico] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[nivel_freatico] thead no encontrado'); return; }
    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[nivel_freatico] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[nivel_freatico] no se encontraron inputs en thead'); return; }
    inputs.forEach(inp => {
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[nivel_freatico] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[nivel_freatico] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[nivel_freatico] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      ...
      console.log('[nivel_freatico] filtro configurado para columna:', col);
    });
    console.log('[nivel_freatico] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  async function load() {
    try {
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      ...
      console.log('[nivel_freatico] filtros limpiados, recargando datos...');
      load();
    });
