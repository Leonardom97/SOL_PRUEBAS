(function(){
  ...
  const COLUMNAS=[
    ...
    'nuevo_operario','error_registro','supervision'
  ];
  ...
  function buildXLSX(rows,name){
    ...
    const body=rows.map(r=>cols.map(c=>r[c]??'')); const ws=XLSX.utils.aoa_to_sheet([head,...body]);
  ...
  function showExportMenu(){
    ...
    const id='exportMenuOficiosPalma'; document.getElementById(id)?.remove();
