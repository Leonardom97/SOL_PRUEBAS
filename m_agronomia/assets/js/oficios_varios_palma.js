...
(function () {
  ...
  const COLUMNAS = [
    ...
    'tipo_labor', 'contratista', 'codigo', 'colaborador', 'personas', 'hora_entrada', 'hora_salida', 'linea_entrada', 'linea_salida',
    'cantidad', 'unidad', 'maquina', 'tractorista', 'nuevo_operario', 'supervision', 'error_registro'
  ];
  const API = 'assets/php/oficios_varios_palma_api.php';
  const ID_KEY = 'oficios_varios_palma_id';
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[oficios_varios_palma] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch oficios_varios_palma' + ' => ' + last);
  }
  function render() {
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[oficios_varios_palma] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[oficios_varios_palma] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[oficios_varios_palma] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[oficios_varios_palma] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[oficios_varios_palma] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[oficios_varios_palma] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[oficios_varios_palma] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  async function save(e) {
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[oficios_varios_palma] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[oficios_varios_palma] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function initFilters() {
    ...
    inputs.forEach(inp => {
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[oficios_varios_palma] input ignored (no col found):', inp);
        return;
      }
      // guardar mapeo
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      ...
      console.log('[oficios_varios_palma] filtros limpiados, recargando datos...');
      load();
    });
