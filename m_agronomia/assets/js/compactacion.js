(function(){
  ...
  const COLUMNAS = [
    'compactacion_id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela',
    'linea','palma','ubicacion','presion','profundidad','error_registro','supervision'
  ];
  ...
  async function save(e){
    ...
    if(idVal){ obj[ID_KEY] = idVal; obj.id = idVal; } // send both to be safe
    ...
    let j = null;
    try { j = await resp.json(); } catch(parseErr){
      try { const txt = await resp.text(); console.warn('save: non-json response', txt); } catch(_) {}
      console.error('save: parse error', parseErr);
      return;
    }
  ...
  async function load(){ try{ const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); } catch(e){ alert(e.message || 'Error cargando compactacion'); } }
