...
(function () {
  ...
  const DOM = {
    tbody: 'tbody-compactacion',
    table: 'tabla-compactacion',
    pagination: 'pagination-compactacion',
    exportBtn: 'exportBtnCompactacion',
    clearBtn: 'clearFiltersBtnCompactacion',
    limitSelect: 'limitSelectCompactacion',
    selectAll: 'selectAllCompactacion',
    form: 'form-editar',
    modal: 'modal-editar'
  };
  const COLUMNAS = [
    'compactacion_id', 'fecha', 'hora', 'colaborador', 'plantacion', 'finca', 'siembra', 'lote', 'parcela', 'linea', 'palma', 'ubicacion',
    'presion', 'profundidad', 'supervision', 'error_registro'
  ];
  ...
  async function fetchData() {
    let last = ''; for (const act of ACTIONS.listFallback) {
      try {
        ...
        console.log('[compactacion] request ->', `${API}?${qs.toString()}`);
    ...
    throw new Error('Error fetch compactacion' + ' => ' + last);
  }
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement) {
    ...
    try {
      ...
      if (j.success) {
        console.log('[compactacion] toggle error_registro ok', { id, action, result: j });
        ...
        if (norm === 'exception' || norm === 'id_required') { console.warn('[compactacion] error suprimido:', err); }
        else { alert(err); }
      ...
      if (norm === 'exception' || norm === 'id_required') { console.warn('[compactacion] error suprimido:', msg); }
      else { alert(msg); }
  ...
  async function inactivar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[compactacion] inactivar ok', { id, result: j }); load(); }
      else {
  ...
  async function activar(id) {
    ...
    try {
      ...
      const j = await r.json();
      if (j.success) { console.log('[compactacion] activar ok', { id, result: j }); load(); }
      else {
  ...
  async function revertir(id) {
    ...
    try {
      ...
      if (!j.success) {
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[compactacion] error suprimido:', err); return; }
        alert(err); return;
      }
      ...
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[compactacion] error suprimido:', msg); return; }
      alert(msg);
    }
  }
  ...
  async function save(e) {
    ...
    // Solo prevenir default si realmente estamos manejando este envío de formulario
    e.preventDefault();
    ...
    try {
      ...
      if (j.success) { alert('Guardado'); load(); setTimeout(() => bootstrap.Modal.getInstance(document.getElementById(DOM.modal))?.hide(), 150); }
        ...
        const norm = (err === null || err === undefined) ? '' : String(err).trim().toLowerCase();
        if (norm === 'exception' || norm === 'id_required') { console.warn('[compactacion] fallo guardar suprimido:', err); }
        else { alert(err); }
      }
    } catch (err) {
      const msg = err?.message || 'Error guardando';
      const norm = (msg === null || msg === undefined) ? '' : String(msg).trim().toLowerCase();
      if (norm === 'exception' || norm === 'id_required') { console.warn('[compactacion] error guardando suprimido:', msg); }
      else { alert(msg); }
    }
  }
  ...
  function initFilters() {
    ...
    if (!table) { console.error('[compactacion] tabla no encontrada:', DOM.table); return; }
    const thead = table.querySelector('thead');
    if (!thead) { console.error('[compactacion] thead no encontrado'); return; }

    const inputs = Array.from(thead.querySelectorAll('input, select, textarea'));
    console.log('[compactacion] inicializando filtros para', inputs.length, 'inputs');
    if (!inputs.length) { console.warn('[compactacion] no se encontraron inputs en thead'); return; }
    ...
    inputs.forEach(inp => {
      ...
      if (!col) {
        // nada mapeado, omitir
        console.debug('[compactacion] input ignored (no col found):', inp);
        return;
      }
      ...
      const handlerDeb = debounce(function (evt) {
        ...
        filters[col] = val;
        page = 1;
        console.log('[compactacion] aplicar filtro:', col, '=', val);
        load();
      }, FILTER_DEBOUNCE_MS);
      ...
      function handlerKey(e) {
        if (e.key === 'Enter') {
          ...
          filters[col] = val;
          page = 1;
          console.log('[compactacion] aplicar filtro (Enter):', col, '=', val);
          load();
        }
      }
      ...
      console.log('[compactacion] filtro configurado para columna:', col);
    });

    console.log('[compactacion] filtros inicializados. Total columnas mapeadas:', Object.keys(filters).length);
  }
  ...
  async function load() {
    try {
      ...
      console.warn('[compactacion] error cargar', e);
    }
  }
  ...
  function init() {
    ...
    document.getElementById(DOM.clearBtn)?.addEventListener('click', () => {
      console.log('[compactacion] limpiando todos los filtros');
      filters = {}; page = 1;
      const table = document.getElementById(DOM.table);
      if (table) {
        const thead = table.querySelector('thead');
      ...
      console.log('[compactacion] filtros limpiados, recargando datos...');
      load();
    });
