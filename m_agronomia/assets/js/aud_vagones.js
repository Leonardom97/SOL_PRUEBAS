...
(function(){
  ...
  const DOM={
    tbody:'tbody-aud-vagones',
    table:'tabla-aud-vagones',
    pagination:'pagination-aud-vagones',
    exportBtn:'exportBtnAudVagones',
    clearBtn:'clearFiltersBtnAudVagones',
    limitSelect:'limitSelectAudVagones',
    selectAll:'selectAllAudVagones',
    form:'form-editar',
    modal:'modal-editar'
  };
  const COLUMNAS=[
    'aud_vagones_id','hora','responsable','fecha_actividad','plantacion','contratista_vagon','tipo','numero_remision',
    'racimos_verdes_vagon','racimos_sobremaduro_vagon','racimos_enfermos_vagon','racimos_tusas_vagon','racimos_pedunculo_largo_vagon',
    'total_racimos','racimos_auditados','tipo_1','tipo_2','tipo_3','tipo_4','supervision','error_registro'
  ];
  ...
  function exportar(t){
    if(t==='todo'){ const qs = new URLSearchParams({action:ACTIONS.listFallback[0], page:1, pageSize:100000}); fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'aud_vagones_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'aud_vagones_'+t);
  }
  function showExportMenu(){
    const btn=document.getElementById(DOM.exportBtn); if(!btn) return;
    const id='exportMenuAudVagones'; document.getElementById(id)?.remove();
  ...
  async function load(){
    try{ const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); }catch(e){ console.warn('[aud_vagones] error cargar', e); }
  }
  function init(){
