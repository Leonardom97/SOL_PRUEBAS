...
(function(){
  ...
  const DOM={
    tbody:'tbody-aud-fertilizacion',
    table:'tabla-aud-fertilizacion',
    pagination:'pagination-aud-fertilizacion',
    exportBtn:'exportBtnAudFertilizacion',
    clearBtn:'clearFiltersBtnAudFertilizacion',
    limitSelect:'limitSelectAudFertilizacion',
    selectAll:'selectAllAudFertilizacion',
    form:'form-editar',
    modal:'modal-editar'
  };
  const COLUMNAS=[
    'aud_fertilizacion_id','hora','responsable','labor_especifica','fecha_actividad','plantacion','siembra','finca',
    'lote','parcela','tipo_auditoria','tipo_fertilizacion','linea_entrada','linea_salida','calificacion','observacion',
    'supervision','error_registro'
  ];
  const API='assets/php/aud_fertilizacion_api.php';
  const ID_KEY='aud_fertilizacion_id';
  ...
  async function toggleErrorRegistro(id, prevWasActive, switchElement){
    ...
    try{
      ...
      if(norm==='exception' || norm==='id_required'){ console.warn('['+ID_KEY+'] error suprimido:', msg); }      else { alert(msg); }
      switchElement.checked = !!prevWasActive;
    }
  }
  ...
  function exportar(t){
    if(t==='todo'){ const qs = new URLSearchParams({action:ACTIONS.listFallback[0], page:1, pageSize:100000}); fetch(`${API}?${qs}`).then(r=>r.json()).then(j=>buildXLSX((j.datos||j.data||[]),'aud_fertilizacion_todo')); return; }
    let rows = data;
    ...
    buildXLSX(rows, 'aud_fertilizacion_'+t);
  }

  function showExportMenu(){
    const btn=document.getElementById(DOM.exportBtn); if(!btn) return;
    const id='exportMenuAudFertilizacion'; document.getElementById(id)?.remove();
  ...
  async function load(){
    try{ const res = await fetchData(); data = res.datos || []; total = res.total || data.length; render(); }catch(e){ console.warn('[aud_fertilizacion] error cargar', e); }
  }
