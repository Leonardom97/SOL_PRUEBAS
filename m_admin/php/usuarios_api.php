 ...
 * usuarios_api.php
 *
 * API para la gestión de Usuarios y Roles del sistema.
 *
 * Funcionalidad:
 *  - CRUD Completo de usuarios (POST: Create/Update, GET: List/Detail, PATCH: Estado).
 *  - Gestión de roles de usuario (adm_usuario_roles).
 *  - Hashing de contraseñas (BCRYPT).
 *  - Validación de duplicados por cédula.
 *
 * Integraciones:
 *  - AuditLogger: Registra creación, edición y cambio de estado de usuarios.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['tipo']) && $input['tipo'] === 'registrar') {
        ...
        $logger->log(
            ...
            ['id_nuevo_usuario' => $new_id, 'cedula' => $cedula]
        );
        header('Content-Type: application/json');
...
if ($_SERVER['REQUEST_METHOD'] === 'PATCH') {
    ...
    $logger->log(
        ...
        ['id_usuario' => $id, 'nuevo_estado' => $estado]
    );
