...
function setupProgressBar(duration, seekCallback) {
    ...
    newContainer.addEventListener('click', (e) => {
        ...
        // Strict Blocking
        if (targetTime > state.maxTimeAllowed + 2) { // Allow 2s buffer
            showErrorToast('Debes responder las preguntas para avanzar.');
            seekCallback(state.maxTimeAllowed);
        } else {
...
function isBlocked() {
    for (let id of state.visibleQuestions) {
        if (!state.answeredQuestions.has(id)) return true;
    }
    return false;
}
...
function checkAnswers() {
    ...
    if (state.visibleQuestions.size === state.data.preguntas.length && !isBlocked()) {
