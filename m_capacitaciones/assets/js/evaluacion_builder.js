...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const idFormulario = params.get('id_formulario');
    ...
    // Event Listeners
    document.getElementById('btn-add-pregunta').addEventListener('click', () => addPregunta());
    document.getElementById('btn-upload-multi').addEventListener('click', uploadMultimedia);
    document.getElementById('btn-add-youtube').addEventListener('click', addYouTubeVideo);

    // Toggle Media Source
    document.querySelectorAll('input[name="media-source"]').forEach(radio => {
        radio.addEventListener('change', (e) => {
            const val = e.target.value;
            document.getElementById('section-upload').style.display = val === 'upload' ? 'block' : 'none';
            document.getElementById('section-youtube').style.display = val === 'youtube' ? 'block' : 'none';
        });
    });

    document.getElementById('form-builder').addEventListener('submit', (e) => {
        e.preventDefault();
        saveEvaluacion(idFormulario);
    });
});
...
function saveEvaluacion(idFormulario) {
    const titulo = document.getElementById('eval-titulo').value;
