...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ------------------------------------------------------------
    // BUSCADOR / FILTRO PARA TODOS LOS TABS
    // ------------------------------------------------------------

    // Función helper para configurar filtros
    function setupFilter(inputId, tableId, countId, cols = [1]) {
      const input = document.getElementById(inputId);
      if (input) {
        input.addEventListener('keyup', function () {
          filterTable(tableId, this.value, cols);
          updateVisibleCount(tableId, countId);
        });
      }
    }

    // Configuración para cada tab
    setupFilter('filtroTemaInput', 'temasTable', 'temasCount');
    setupFilter('filtroProcesoInput', 'procesoTable', 'procesoCount');
    setupFilter('filtroLugarInput', 'lugarTable', 'lugarCount');
    setupFilter('filtroTActividadInput', 'tactividadTable', 'tactividadCount');
    // Para relacion proceso-tema buscamos en cols 1 (Proceso) y 2 (Tema)
    setupFilter('filtroProcesoTemaInput', 'procesoTemaTable', 'procesoTemaCount', [1, 2]);

    // Helper para filtrar tabla
    function filterTable(tableId, query, colsIds) {
      ...
      rows.forEach(row => {
        let match = false;
        // Revisar cada columna configurada
        colsIds.forEach(colIndex => {
    ...
    // Actualiza el contador con filas visibles
    // Se mantiene igual, pero la incluimos en el bloque reemplazado para limpieza
    function updateVisibleCount(tableId, countId) {
      const table = document.getElementById(tableId);
