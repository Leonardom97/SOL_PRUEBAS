 ...
 *
 * API para la gestión de relaciones entre Procesos y Temas de capacitación.
 * 
 * Funcionalidad:
 *  - Gestión Muchos a Muchos (M:N) con estado activo/inactivo.
 *  - list: Listado completo de relaciones.
 *  - add/delete/toggle: CRUD de vinculaciones.
 *  - get_temas_by_proceso: Temas asociados a un proceso (para selectores en cascada).
 *
 * Integraciones:
 *  - AuditLogger.
 */
...
try {
    switch ($action) {
        ...
        case 'add':
            ...
            if ($exists) {
                if ($exists['activo']) {
                    ...
                    $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'system', 'ACTIVATE_PROCESO_TEMA', 'Relación proceso-tema reactivada', ['id' => $exists['id']]);
            ...
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'system', 'CREATE_PROCESO_TEMA', 'Relación proceso-tema creada', ['id' => $result['id']]);
        ...
        case 'delete':
            ...
            $logAction = $hard_delete ? 'DELETE_PROCESO_TEMA' : 'INACTIVATE_PROCESO_TEMA';
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'system', $logAction, 'Relación proceso-tema eliminada/inactivada', ['id' => $id, 'hard_delete' => $hard_delete]);
        ...
        case 'toggle':
            ...
            $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'system', 'TOGGLE_PROCESO_TEMA', 'Relación proceso-tema toggle', ['id' => $id, 'activo' => $result['activo']]);
