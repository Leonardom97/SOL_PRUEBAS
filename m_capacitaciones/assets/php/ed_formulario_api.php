 ...
 *
 * API para la gestión de formularios de capacitaciones (Edición, Lectura, Listado).
 *
 * Funcionalidad:
 *  - Leer datos de formulario existente (incluyendo asistentes y metadatos).
 *  - Actualizar formulario (CRUD).
 *  - Listar formularios con paginación y búsqueda (para DataTables).
 *  - Gestión de asistentes (Agregar, Eliminar).
 *  - Integración con AuditLogger para trazabilidad.
 *
 * Acciones soportadas:
 *  - leer_formulario, listar, eliminar, buscar_colaborador, agregar_asistente, actualizar_formulario, eliminar_asistente.
 */
...
if ($action === 'eliminar') {
    ...
    if ($id > 0) {
        ...
        $logger->log(
            ...
            'DELETE_FORM',
            'Eliminación de formulario de capacitación',
            ['id_formulario' => $id]
        );
...
if ($action === 'agregar_asistente') {
    ...
    $logger->log(
        ...
        $_SESSION['tipo_usuario'] ?? 'colaborador',
        'ADD_ASISTENTE',
        'Agregado asistente a formulario',
        ['id_formulario' => $id_formulario, 'cedula_asistente' => $cedula]
    );
...
if ($action === 'actualizar_formulario') {
    ...
    if ($id > 0) {
        ...
        $logger->log(
            ...
            $_SESSION['tipo_usuario'] ?? 'colaborador',
            'UPDATE_FORM',
            'Actualización de formulario de capacitación',
            ['id_formulario' => $id]
        );
        respond(['ok' => true]);
    } else {
