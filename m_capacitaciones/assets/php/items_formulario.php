 ...
 *
 * API unificada para la gestión CRUD de tablas auxiliares (Catálogos).
 * Soporta operaciones para: Lugares, Procesos, Temas, Tipos de Actividad.
 *
 * Funcionalidad:
 *  - ADD: Insertar nuevos registros.
 *  - UPDATE: Modificar nombres.
 *  - ACTIVAR/INACTIVAR: Cambiar estado (0: Activo, 1: Inactivo).
 *  - Inferencia automática de columnas de texto y tablas.
 *
 * Integraciones:
 *  - AuditLogger.
 *  - Compatible con JSON body y Form Data.
 */
...
function api_log($m)
{
    @file_put_contents(__DIR__ . '/php_error_api.log', "[" . date('Y-m-d H:i:s') . "] " . $m . "\n", FILE_APPEND | LOCK_EX);
}
...
if (!$ok || !isset($pg) || !($pg instanceof PDO)) {
    api_log("items_formulario: DB no disponible");
...
$id_raw = $getv('id', '');
$state_raw = $getv('state', $getv('estado', '')); // allow 'state' or 'estado'
...
$map = [
    ...
    'cap_lugar' => 'cap_lugar',
    'proceso' => 'cap_proceso',
    'cap_proceso' => 'cap_proceso',
    'tema' => 'cap_tema',
    'cap_tema' => 'cap_tema',
    'tipo' => 'cap_tipo_actividad',
    'tactividad' => 'cap_tipo_actividad',
    'cap_tipo_actividad' => 'cap_tipo_actividad',
    'tipo_actividad' => 'cap_tipo_actividad',
    'tipoactividad' => 'cap_tipo_actividad'
];
...
try {
    ...
    if ($action === 'add') {
        ...
        if (!preg_match('/^[a-zA-Z0-9_]+$/', $nameCol)) {
            api_log("items_formulario add: columna insegura {$nameCol} en {$table}");
            respond(['success' => false, 'error' => 'Configuración inválida']);
        }
        ...
        $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'system', 'CREATE_ITEM_' . strtoupper($table), 'Registro creado en ' . $table, ['id' => $r['id'], 'nombre' => $r['nombre']]);
    ...
    if ($action === 'activar' || $action === 'inactivar') {
        ...
        $logAction = ($action === 'activar') ? 'ACTIVATE_ITEM_' . strtoupper($table) : 'INACTIVATE_ITEM_' . strtoupper($table);
        $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'system', $logAction, ucfirst($action) . ' registro en ' . $table, ['id' => $r['id']]);
    ...
    if ($action === 'update') {
        ...
        if (!$cols) {
            api_log("items_formulario update: tabla {$table} sin columnas");
            respond(['success' => false, 'error' => 'Tabla no encontrada']);
        }
        ...
        foreach ($prefs as $p) {
            foreach ($cols as $c) {
                if (stripos($c['column_name'], $p) !== false) {
                    $col = $c['column_name'];
                    break 2;
                }
            }
        ...
        if (!$col) {
            foreach ($cols as $c) {
                ...
                if ($c['column_name'] !== 'id' && !in_array($dt, ['integer', 'bigint', 'smallint', 'serial', 'bigserial', 'int4', 'int8', 'int2'])) {
                    $col = $c['column_name'];
                    break;
                }
            }
        ...
        if (!$col) {
            api_log("items_formulario update: no text column for {$table}");
            respond(['success' => false, 'error' => 'Tabla no preparada para edición']);
        }

        // Seguridad: asegurar que el nombre de columna es seguro (sin caracteres raros)
        if (!preg_match('/^[a-zA-Z0-9_]+$/', $col)) {
            api_log("items_formulario update: columna insegura {$col} en {$table}");
            respond(['success' => false, 'error' => 'Configuración inválida']);
        }
        ...
        $logger->log($_SESSION['usuario_id'] ?? null, $_SESSION['tipo_usuario'] ?? 'system', 'UPDATE_ITEM_' . strtoupper($table), 'Actualización en ' . $table, ['id' => $row['id'], 'nombre' => $row[$col]]);
